...
const Footer = () => (
  <div className="w-full py-8 flex flex-col items-center gap-1 border-t border-white/[0.06] mt-8">
    ...
    <p className="text-[9px] text-white/20">Anon • Nested • Dark • White • Push • Share</p>
  </div>
);
...
export default function YakFixed(){
  ...
  const handleShare = async () => {
    ...
    if(navigator.share){ await navigator.share({title:"SRET ANON TALK", text:"Reply to Reply 100 percent Anonymous SRET Only", url}); }
...
const renderComment = (c:any, depth=0) => {
  ...
  return(
    <div className="min-h-screen bg-[#0a0a0b] text-white flex flex-col">
      ...
      {showNotif && <div className="fixed top-[60px] right-4 z-30 w-[320px] max-h-[400px] overflow-y-auto bg-[#141416] border border-white/10 rounded-2xl p-3 space-y-2 shadow-2xl"><div className="flex justify-between items-center mb-2"><p className="text-xs font-bold">Notifications 🔔 Push</p><button onClick={()=>setShowNotif(false)} className="text-[10px] text-white/40">Close</button></div>{notifs.length===0? <p className="text-xs text-white/30">No notifications - Reply vasthe ikkada bell vastundi</p> : notifs.map((n:any)=><div key={n.id} className="bg-white/[0.05] p-3 rounded-xl text-xs border border-white/[0.06]"><p className="text-white/90">{n.text}</p></div>)}</div>}

      <div className="max-w-[600px] mx-auto w-full flex-1 p-3 pb-[84px] space-y-3">
        {feedTab==='top'? (
          <div className="space-y-3"><div className="bg-white/[0.05] border-2 border-white/10 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">{userData.college} TOP ANON - ANALYTICS</p><p className="font-black mt-1 text-white">Top Anonymous - Nested</p><div className="grid grid-cols-3 gap-2 mt-3"><div className="bg-black border border-white/10 rounded-xl p-2"><p className="text-[8px] text-white/30">USERS</p><p className="font-black text-sm">{totalUsers}</p></div><div className="bg-black border border-white/10 rounded-xl p-2"><p className="text-[8px] text-white/30">YAKS</p><p className="font-black text-sm">{yaks.length}</p></div><div className="bg-white text-black rounded-xl p-2"><p className="text-[8px]">NOTIFS</p><p className="font-black text-sm">{notifs.length}</p></div></div></div>
            {leaderboard.map((u:any,i:number)=><div key={u.id} className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><span className="w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-white">{i+1}</span><span className="w-9 h-9 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">👻</span><div><p className="font-bold text-[13px] text-white">Anonymous Student {i+1}</p><p className="text-[10px] text-white/40">{u.totalPosts||0} posts • {u.yakarma} karma</p></div></div><p className="font-black text-sm text-white">{u.yakarma}</p></div>)}<Footer/></div>
        ) : (
          <>
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-10 h-10 bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white">👻</div><div><p className="font-bold text-[13px] text-white">Anonymous plus Nested Reply - {userData.college} ONLY</p><p className="text-[11px] text-white/40">Reply to reply thread - White letters - Push enabled</p></div></div><span className="px-3 py-1.5 rounded-full bg-white text-black text-[10px] font-bold">NESTED</span></div>
                ...
                <div key={y.id} className={`bg-white/[0.04] border-2 rounded-[20px] p-5 ${isOwn?'border-white/20 bg-white/[0.06]':'border-white/10'}`}>
                  ...
                  {activePost===y.id && (
                    <div className="mt-5 border-t-2 border-white/10 pt-4 space-y-1">
                      <p className="text-[10px] font-bold tracking-widest text-white/30 mb-3">NESTED REPLIES - {comments.length} - WHITE - PUSH ENABLED</p>
                      {replyTo && (
        ...
        <div className="fixed inset-0 bg-[#0a0a0b] z-40 flex flex-col overflow-hidden">
          ...
          <div className="max-w-[600px] mx-auto w-full flex flex-col h-full bg-[#0a0a0b]">
            <div className="p-5 flex items-center justify-between border-b-2 border-white/10">
              ...
              <div className="text-center"><p className="text-[11px] font-bold tracking-widest text-white">ANON NESTED - {userData.college}</p><p className="text-[10px] text-white/30">Dark bg White letters • Push ready</p></div>
            ...
            <div className="p-6 flex-1 overflow-y-auto bg-[#0a0a0b]">
              <div className="flex gap-3 mb-6"><div className="w-11 h-11 bg-white/5 border-2 border-white/10 rounded-full flex items-center justify-center text-white">A</div><div><p className="font-bold text-[14px] text-white">Anonymous - {userData.college} - Nested Enabled</p><p className="text-[11px] text-white/40">Post comments have reply to reply thread • Share enabled</p></div></div>
      ...
      {showProfile && (<div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><div className="flex gap-4"><div className="w-[72px] h-[72px] bg-white/5 border-2 border-white/10 rounded-[20px] flex items-center justify-center text-3xl text-white">A</div><div className="flex-1"><h2 className="font-black text-[16px] leading-none text-white">Anonymous - {userData.college} - NESTED</h2><p className="text-[11px] text-white/40 mt-2 leading-[1.4]">Nested Reply + Push + Share + Analytics - White letters - Dark bg</p><div className="flex gap-2 mt-4 flex-wrap"><span className="px-3 py-1.5 bg-white text-black rounded-full text-[10px] font-bold">{userData.yakarma} karma</span><span className="px-3 py-1.5 bg-white/10 border border-white/10 text-white rounded-full text-[9px] font-bold">{userData.college} ONLY NESTED WHITE</span></div></div></div>
