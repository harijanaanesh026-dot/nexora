...
const Footer = () => (
  <div className="w-full py-8 flex flex-col items-center gap-1 border-t border-white/[0.06] mt-8">
    ...
    <p className="text-[9px] text-white/20">Anon • Insta Like ❤️ • Nested • White</p>
  </div>
);
...
export default function YakFixed(){
    ...
    return onAuthStateChanged(auth, async(u:any)=>{
      if(u){
        ...
        if(snap.empty){
          ...
          await addDoc(collection(db,'users'),{ uid:u.uid, email:u.email||'', username:anonName, avatar:localStorage.getItem('selected_avatar')||'👻', college:String(col), collegeEmail:String(localStorage.getItem('college_email')||''), rollNumber:String(localStorage.getItem('roll_number')||''), yakarma:100, totalPosts:0, likedPosts:[], savedPosts:[], dislikedPosts:[], pollVoted:[], reportedPosts:[], createdAt:serverTimestamp() });
  ...
  const handleLike=async(y:any)=>{
    ...
    setLikedAnim(prev=>({...prev, [y.id]: true})); setTimeout(()=> setLikedAnim(prev=>({...prev, [y.id]: false})), 350);
    try{
  ...
  const handleDoubleTap=(y:any)=>{
    ...
    if(now-last<300){
      setDoubleTapHearts(prev=>({...prev, [y.id]: true})); setTimeout(()=> setDoubleTapHearts(prev=>({...prev, [y.id]: false})), 1000);
      if(!userData.likedPosts?.includes(y.id)) handleLike(y);
    }
    lastTapRef.current[y.id]=now;
  };
  const handleSave=async(y:any)=>{
    if(!userData) return; const isSaved=userData.savedPosts?.includes(y.id); const userRef=doc(db,'users',userData.id);
    try{
      if(isSaved){ await updateDoc(userRef,{savedPosts:arrayRemove(y.id)}); setUserData({...userData, savedPosts:userData.savedPosts.filter((i:string)=>i!==y.id)}); showToast("Unsaved"); }
      else{ await updateDoc(userRef,{savedPosts:arrayUnion(y.id)}); setUserData({...userData, savedPosts:[...(userData.savedPosts||[]), y.id]}); showToast("Saved 🔖"); }
    }catch{}
  };
  ...
  const handlePost=async()=>{
    ...
    try{
      ...
      await addDoc(collection(db,'yaks'),payload); await updateDoc(doc(db,'users',userData.id),{totalPosts:increment(1), yakarma:increment(5)}); setNewYak(''); setYakImage(''); setPollOptions(['','']); setYakType('yak'); setScreen('feed'); showToast("Posted ❤️");
  ...
  const handleCommentPost=async(yId:string)=>{
    ...
    const payload:any={ text, uid:user.uid, username:"Anonymous - "+userData.college, avatar:"👻", parentId:replyTo? replyTo.id:null, replyToUsername:replyTo? replyTo.username:null, createdAt:serverTimestamp() };
  ...
  if(screen==='college'){
    return(
      <div className="min-h-screen bg-[#0a0a0b] text-white"><style>{`body{background:#0a0a0b} ::-webkit-scrollbar{display:none} *{-webkit-tap-highlight-color:transparent}`}</style>
        ...
        <div className="max-w-md mx-auto p-6 bg-[#0a0a0b] min-h-screen"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-white text-black rounded-xl flex items-center justify-center font-black">Y</div><div><p className="font-black text-sm tracking-wide">ANONYMOUS COLLEGE TALK</p><p className="text-[10px] text-white/40">{totalUsers} anonymous • Insta Like ❤️ • Nested</p></div></div>
          <h1 className="text-[36px] font-black mt-8 leading-[0.9] tracking-tight">Anonymous<br/>Insta Like ❤️<br/><span className="text-white/30">College Talk</span></h1><p className="text-[13px] text-white/50 mt-3">Double tap to like - Instagram style - Reply to reply thread - White letters</p>
...
const renderComment = (c:any, depth=0) => {
          ...
          <div className="flex-1">
            <div className="bg-white/[0.05] border border-white/10 rounded-[14px] px-4 py-2.5">
              <div className="flex gap-2 items-center">
                ...
                {isReply && <span className="px-2 py-0.5 bg-white/10 rounded-full text-[7px] text-white/50 font-bold">REPLY ❤️</span>}
              </div>
            ...
            <div className="flex gap-3 mt-1.5 ml-1 items-center">
              <button onClick={()=>setReplyTo(c)} className="text-[11px] font-bold text-white/30 hover:text-white">Reply ❤️</button>
            </div>
  ...
  return(
    <div className="min-h-screen bg-[#0a0a0b] text-white flex flex-col">
      <style>{`
        body{background:#0a0a0b} ::-webkit-scrollbar{display:none} *{-webkit-tap-highlight-color:transparent}
        @keyframes heartBurst { 0%{transform:scale(0); opacity:0} 15%{transform:scale(1.3); opacity:1} 30%{transform:scale(1); opacity:1} 80%{transform:scale(1); opacity:1} 100%{transform:scale(1); opacity:0} }
     ...
     .like-pop{ animation: likePop 0.35s ease-out; }
      ...
      <div className="sticky top-0 z-20 bg-[#0a0a0b]/80 backdrop-blur-2xl border-b border-white/10">
        <div className="max-w-[600px] mx-auto px-4 h-14 flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-white text-black rounded-xl flex items-center justify-center font-black text-sm">Y</div><div><p className="font-bold text-[13px] leading-none text-white">{userData.college} ANON ❤️ {yaks.length}</p><p className="text-[10px] text-white/40">Insta Like • Double Tap • Nested</p></div></div><button onClick={()=>setShowProfile(true)} className="w-9 h-9 bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white">👻</button></div>
        <div className="max-w-[600px] mx-auto px-3 pb-3 flex gap-2 overflow-x-auto">
          <button onClick={()=>setFeedTab('new')} className={`h-9 px-4 rounded-full text-xs font-bold border-2 whitespace-nowrap ${feedTab==='new'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>NEW {yaks.length} ❤️</button>
          ...
          <button onClick={()=>setFeedTab('hot')} className={`h-9 px-4 rounded-full text-xs font-bold border-2 whitespace-nowrap ${feedTab==='hot'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>HOT ❤️</button>
      ...
      <div className="max-w-[600px] mx-auto w-full flex-1 p-3 pb-[84px] space-y-3">
        {feedTab==='top'? (
          <div className="space-y-3"><div className="bg-white/[0.05] border-2 border-white/10 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">{userData.college} TOP ANON - INSTA LIKE ❤️</p><p className="font-black mt-1 text-white">Top Liked - Insta ❤️</p></div>
            {leaderboard.map((u:any,i:number)=><div key={u.id} className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><span className="w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-white">{i+1}</span><span className="w-9 h-9 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">👻</span><div><p className="font-bold text-[13px] text-white">Anonymous Student {i+1}</p><p className="text-[10px] text-white/40">{u.totalPosts||0} posts • ❤️ {u.likedPosts?.length||0} liked</p></div></div><p className="font-black text-sm text-white">{u.yakarma} ❤️</p></div>)}<Footer/></div>
        ) : (
          <>
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-10 h-10 bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white">❤️</div><div><p className="font-bold text-[13px] text-white">Insta Like + Nested - {userData.college} ONLY</p><p className="text-[11px] text-white/40">Double tap ❤️ • Reply to reply • Save 🔖</p></div></div><span className="px-3 py-1.5 rounded-full bg-white text-black text-[10px] font-bold">❤️ NESTED</span></div>

            {(feedTab==='new'? yaks : feedTab==='meme'? memeYaks : hotYaks).map(y=>{
              const liked=userData.likedPosts?.includes(y.id); const score=(y.likes||0); const isOwn=user?.uid===y.uid; const isPoll=y.type==='poll'; const hasVoted=userData.pollVoted?.includes(y.id); const nestedTree = activePost===y.id? buildTree(comments) : []; const isSaved=userData.savedPosts?.includes(y.id);
              return(
                <div key={y.id} className={`bg-white/[0.04] border-2 rounded-[20px] p-5 relative overflow-hidden ${isOwn?'border-white/20 bg-white/[0.06]':'border-white/10'}`}>
                  <div className="flex justify-between items-start"><div className="flex gap-3"><div className="w-9 h-9 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-sm text-white">👻</div>
                      <div><div className="flex gap-2 items-center flex-wrap"><p className="font-bold text-[13px] text-white">Anonymous - {y.college} {isOwn? '- YOU' : ''}</p><span className="px-2 py-0.5 rounded-full text-[8px] font-bold bg-white text-black">{y.college} ONLY ❤️</span></div><p className="text-[10px] text-white/30 mt-0.5">Anon • {score} likes • Nested</p></div></div>
                  ...
                  <div className="relative mt-4 select-none" onDoubleClick={()=>handleDoubleTap(y)} onTouchEnd={()=>handleDoubleTap(y)} onClick={()=>handleDoubleTap(y)}>
                    ...
                    {doubleTapHearts[y.id] && <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10"><div className="text-[110px] heart-burst drop-shadow-2xl">❤️</div></div>}
                  </div>
                  ...
                  <div className="mt-5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <button onClick={()=>handleLike(y)} className={`${likedAnim[y.id]?'like-pop':''} text-[26px] leading-none select-none`}>{liked? '❤️' : '🤍'}</button>
                        <button onClick={()=>{ setActivePost(activePost===y.id?null:y.id); setReplyTo(null); }} className="text-[22px] leading-none">💬</button>
                        <button onClick={()=>{ navigator.clipboard.writeText(window.location.href); showToast("Link copied"); }} className="text-[20px] leading-none">↗️</button>
                      </div>
                      <button onClick={()=>handleSave(y)} className="text-[22px] leading-none">{isSaved? '🔖' : '📑'}</button>
                    </div>
                    <div className="mt-3">
                      <p className="text-[13px] font-black">{score} likes</p>
                      {score>0 && (
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex -space-x-1.5">
                            <div className="w-5 h-5 rounded-full bg-yellow-400 border-2 border-[#0a0a0b]"></div>
                            <div className="w-5 h-5 rounded-full bg-purple-500 border-2 border-[#0a0a0b]"></div>
                            <div className="w-5 h-5 rounded-full bg-green-500 border-2 border-[#0a0a0b]"></div>
                          </div>
                          <p className="text-[11px] text-white/50">Liked by <span className="text-white font-bold">Anonymous Owl</span> and <span className="text-white font-bold">{Math.max(0,score-1)} others</span></p>
                        </div>
                      )}
                      <p className="text-[10px] text-white/30 mt-1 uppercase tracking-widest">Double tap to like ❤️ • {y.commentsCount||0} comments • Nested • Save 🔖</p>
                    </div>
                  </div>

                  {activePost===y.id && (
                    <div className="mt-5 border-t-2 border-white/10 pt-4 space-y-1">
                      <p className="text-[10px] font-bold tracking-widest text-white/30 mb-3">NESTED REPLIES - {comments.length} - INSTA LIKE ❤️</p>
                      {replyTo && (
                      ...
                      <div className="max-h-[420px] overflow-y-auto pr-1">
                        {nestedTree.length===0 && <p className="text-xs text-white/20 text-center py-8">No comments yet - Be first nested ❤️</p>}
                        {nestedTree.map((c:any)=>renderComment(c,0))}
                      </div>
                      <div className="flex gap-2.5 mt-4">
                        <input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder={replyTo? `Reply to ${replyTo.username} ❤️` : "Add a comment... ❤️"} className="flex-1 bg-white/5 border-2 border-white/10 rounded-full px-5 h-11 text-[13px] outline-none text-white placeholder:text-white/30 focus:border-white" onKeyDown={e=>{ if(e.key==='Enter'){ handleCommentPost(y.id); } }}/>
            ...
            {(feedTab==='new'? yaks : feedTab==='meme'? memeYaks : hotYaks).length===0 && <div className="py-24 text-center bg-white/[0.03] border-2 border-white/10 rounded-[24px]"><div className="w-24 h-24 bg-white/5 border-2 border-white/10 rounded-[24px] mx-auto flex items-center justify-center text-4xl">❤️</div><p className="font-black mt-6 text-[18px] text-white">No anon talk yet</p><p className="text-xs text-white/30 mt-1">Double tap to like - Instagram style ❤️</p><button onClick={()=>setScreen('create')} className="mt-6 bg-white text-black px-8 h-11 rounded-full text-[13px] font-bold">First Post ❤️</button></div>}
            <Footer/>
          </>
        )}
      </div>
      <div className="fixed bottom-0 left-0 right-0 bg-[#0a0a0b]/90 backdrop-blur-2xl border-t-2 border-white/10"><div className="max-w-[600px] mx-auto px-6 h-[72px] flex items-center justify-between"><button onClick={()=>setFeedTab('new')} className="flex flex-col items-center gap-1.5"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-bold ${feedTab==='new'?'bg-white text-black':'bg-white/5 text-white/30 border border-white/10'}`}>❤️</div><span className="text-[8px] font-bold tracking-widest text-white/30">FEED {yaks.length}</span></button><button onClick={()=>setScreen('create')} className="w-[56px] h-[56px] bg-white text-black rounded-full flex items-center justify-center text-[24px] font-black">+</button><button onClick={()=>setShowProfile(true)} className="flex flex-col items-center gap-1.5"><div className="w-7 h-7 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-xs text-white">P</div><span className="text-[8px] font-bold tracking-widest text-white/30">ANON {userData?.yakarma} ❤️</span></button></div></div>
        ...
        <div className="fixed inset-0 bg-[#0a0a0b] z-40 flex flex-col overflow-hidden">
          ...
          <div className="max-w-[600px] mx-auto w-full flex flex-col h-full bg-[#0a0a0b]">
            <div className="p-5 flex items-center justify-between border-b-2 border-white/10">
              ...
              <div className="text-center"><p className="text-[11px] font-bold tracking-widest text-white">ANON NESTED - {userData.college} ❤️</p><p className="text-[10px] text-white/30">Insta Like • White letters</p></div>
              <button onClick={handlePost} disabled={posting||(!newYak.trim()&&!yakImage)} className={`px-6 h-10 rounded-full font-bold text-[13px] ${posting||(!newYak.trim()&&!yakImage)?'bg-white/5 text-white/20 border-2 border-white/5':'bg-white text-black'}`}>{posting?'Posting...':'Post ❤️'}</button>
            </div>
            <div className="p-3 flex gap-2 border-b-2 border-white/5 overflow-x-auto bg-white/[0.02]">
              <button onClick={()=>setYakType('yak')} className={`px-5 h-9 rounded-full text-xs font-bold border-2 whitespace-nowrap ${yakType==='yak'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>Talk ❤️</button>
            ...
            <div className="p-6 flex-1 overflow-y-auto bg-[#0a0a0b]">
              <div className="flex gap-3 mb-6"><div className="w-11 h-11 bg-white/5 border-2 border-white/10 rounded-full flex items-center justify-center text-white">A</div><div><p className="font-bold text-[14px] text-white">Anonymous - {userData.college} - Nested + Insta ❤️</p><p className="text-[11px] text-white/40">Double tap like + Reply to reply thread</p></div></div>
              <textarea value={newYak} onChange={e=>setNewYak(e.target.value)} placeholder={`Talk about ${userData.college} anonymously... Double tap ❤️ will work... Nested thread... White letters`} autoFocus className="w-full bg-transparent text-[19px] leading-[1.45] outline-none placeholder:text-white/20 resize-none min-h-[120px] text-white" maxLength={300}/>
              ...
              <div className="mt-6">
                {yakImage? (
                  ...
                  <label className="w-full border-2 border-dashed border-white/10 rounded-[16px] p-8 flex flex-col items-center justify-center cursor-pointer hover:border-white/20 bg-white/[0.02]">
                    ...
                    <span className="text-xs font-bold text-white/60">Upload Image - Insta Like ❤️ Nested - White</span>
                    <span className="text-[10px] text-white/30 mt-1">Max 800KB • Double tap ❤️ will work</span>
                    <input type="file" hidden accept="image/*" onChange={e=>handleImageUpload(e,setYakImage)} />
                  </label>
                )}
              </div>
            </div>
            <div className="p-5 border-t-2 border-white/5 bg-white/[0.02]"><div className="bg-white/[0.05] border-2 border-white/10 rounded-xl p-4 flex gap-3 items-center"><div className="w-2 h-2 bg-white rounded-full animate-pulse"></div><p className="text-[11px] text-white/50"><span className="font-bold text-white">INSTA + NESTED ❤️:</span> Double tap ❤️ blast + Reply to reply thread - Dark bg - White letters</p></div><Footer/></div>
          </div>
        </div>
      )}

      {editingPost && <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center p-4"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 pb-8 shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><h3 className="font-black text-[16px] text-white">Edit Anon Nested ❤️</h3><textarea value={editText} onChange={e=>setEditText(e.target.value)} className="w-full mt-5 bg-white/[0.05] border-2 border-white/10 rounded-xl p-4 text-[15px] outline-none min-h-[120px] resize-none text-white focus:border-white"/><div className="flex gap-3 mt-6"><button onClick={()=>{ setEditingPost(null); setEditText(''); }} className="flex-1 h-12 bg-white/5 border-2 border-white/10 rounded-full font-bold text-xs text-white">Cancel</button><button onClick={handleEdit} disabled={!editText.trim()} className={`flex-1 h-12 rounded-full font-bold text-xs ${!editText.trim()?'bg-white/5 text-white/20 border-2 border-white/5':'bg-white text-black'}`}>Save ❤️</button></div></div></div>}

      {showProfile && (<div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><div className="flex gap-4"><div className="w-[72px] h-[72px] bg-white/5 border-2 border-white/10 rounded-[20px] flex items-center justify-center text-3xl text-white">❤️</div><div className="flex-1"><h2 className="font-black text-[16px] leading-none text-white">Anonymous - {userData.college} - INSTA ❤️ NESTED</h2><p className="text-[11px] text-white/40 mt-2 leading-[1.4]">Insta Like • Double tap • Save • Liked by • Nested • White letters • Dark bg</p><div className="flex gap-2 mt-4 flex-wrap"><span className="px-3 py-1.5 bg-white text-black rounded-full text-[10px] font-bold">{userData.yakarma} karma ❤️</span><span className="px-3 py-1.5 bg-white/10 border border-white/10 text-white rounded-full text-[9px] font-bold">{userData.college} ONLY NESTED WHITE ❤️</span><span className="px-3 py-1.5 bg-red-500/20 border border-red-500/20 text-red-300 rounded-full text-[9px] font-bold">❤️ {userData.likedPosts?.length||0} LIKED</span><span className="px-3 py-1.5 bg-yellow-500/20 border border-yellow-500/20 text-yellow-300 rounded-full text-[9px] font-bold">🔖 {userData.savedPosts?.length||0} SAVED</span></div></div></div>
      <div className="grid grid-cols-3 gap-3 mt-6"><div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 text-center"><p className="font-black text-xl text-white">{userData.totalPosts||0}</p><p className="text-[9px] font-bold tracking-widest text-white/30 mt-1">POSTS</p></div><div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 text-center"><p className="font-black text-xl text-white">{userData.likedPosts?.length||0}</p><p className="text-[9px] font-bold tracking-widest text-white/30 mt-1">LIKED ❤️</p></div><div className="bg-white text-black rounded-[18px] p-4 text-center"><p className="font-black text-xl">{userData.savedPosts?.length||0}</p><p className="text-[9px] font-bold tracking-widest mt-1">SAVED 🔖</p></div></div>
      <div className="mt-6 bg-white/[0.03] border border-white/10 rounded-[16px] p-4"><p className="text-[11px] font-bold text-white">How Nested + Insta ❤️ Works:</p><p className="text-[11px] text-white/40 mt-2 leading-[1.6]">Double tap anywhere on post - Big ❤️ blast - 110px animation - Like count + Avatars - Liked by Anonymous Owl and X others - Save 🔖 - Comment - Reply - Banner Replying to - parentId - buildTree - recursive renderComment - depth ml-6 border-l-2</p></div>
      <button onClick={()=>{auth.signOut(); localStorage.clear(); window.location.reload();}} className="w-full mt-6 bg-white/5 border-2 border-white/10 h-12 rounded-full text-xs font-bold text-white/60">Logout</button><button onClick={()=>setShowProfile(false)} className="w-full mt-3 bg-white text-black h-12 rounded-full font-bold text-xs">Close ❤️ Nested White</button><div className="mt-4"><Footer/></div></div></div>)}
    </div>
  );
      }
