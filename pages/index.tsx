    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md w-96">
          ...
          <button onClick={() => setPage("dashboard")} className="w-full bg-purple-600 text-white py-2 rounded">Continue</button>
        </div>
      </div>
    );
  }
  ...
  if (page === "dashboard") {
    return (
      <div className="min-h-screen bg-gray-50">
        <header className="bg-white shadow p-4"><h1 className="text-2xl font-bold text-purple-600">Nexora</h1></header>
        <main className="p-8">
          <h2 className="text-3xl font-bold mb-6">Dashboard</h2>
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-xl font-bold mb-4">Video Room</h3>
            <input value={channelName} onChange={(e) => setChannelName(e.target.value)} className="border p-2 rounded w-full mb-4" placeholder="Enter Room Name"/>
            <button onClick={handleJoin} className="bg-green-600 text-white px-6 py-2 rounded font-bold">Join Video Room</button>
          </div>
        </main>
      </div>
    );
