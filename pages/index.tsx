...
import { Home, Search, Bell, User, Plus, Flame, Star, Award, Edit, Save, MessageCircle, Heart, Share2, BookOpen, Rocket, Users, LogOut, Bookmark, Trophy, Camera, Trash2, Users2, X } from "lucide-react";
...
function FeedTab({user}: any) {
  ...
  useEffect(() => { let q = query(collection(db, "posts"), orderBy("createdAt", "desc"), limit(50)); return onSnapshot(q, (snap) => { let allPosts: any[] = snap.docs.map((d) => ({ id: d.id,...d.data() as any })); if(feedType === "personalized" && following.length > 0) allPosts = allPosts.filter(p => [...following, user?.uid].includes(p.userId)); if(feedType === "trending") allPosts = allPosts.sort((a, b) => (b.likes?.length || 0) + (b.shares || 0)*2 - (a.likes?.length || 0) - (a.shares || 0)*2).slice(0, 20); if(feedType === "growth") allPosts = allPosts.filter(p => p.text?.includes("#Learning") || p.text?.includes("#Startup") || p.text?.includes("#Skills") || p.text?.includes("#Achievement")); setPosts(allPosts); }) }, [following, user, feedType]);

  const handleCreatePost = async () => {
    if(!newPost.trim() &&!postImage) return;
    let imageUrl = "";
    if(postImage) { const storageRef = ref(storage, `posts/${Date.now()}-${postImage.name}`); const snap = await uploadBytes(storageRef, postImage); imageUrl = await getDownloadURL(snap.ref); }
  ...
  return (
    <div>
      <div className="flex gap-3 mb-4 overflow-x-auto pb-2">
        <div onClick={() => storyInputRef.current?.click()} className="w-16 h-20 rounded-lg border-2 border-dashed border-gray-600 flex items-center justify-center cursor-pointer"><Plus /></div>
        <input ref={storyInputRef} type="file" accept="image/*" onChange={handleAddStory} className="hidden"/>
        {stories.map(s => <div key={s.id} className="w-16 h-20 rounded-lg overflow-hidden"><img src={s.image} className="w-full h-full object-cover"/></div>)}
      </div>
      ...
      <div className="border rounded-xl p-4 mb-6 bg-gray-900 border-gray-800"><textarea value={newPost} onChange={e => setNewPost(e.target.value)} placeholder="Share your #Learning #Achievement..." className="w-full bg-gray-800 p-3 rounded-lg outline-none" rows={3} /><div className="flex justify-between mt-3"><input type="file" onChange={e => setPostImage(e.target.files?.[0] || null)} accept="image/*" /><button onClick={handleCreatePost} className="bg-blue-500 px-4 py-2 rounded-lg flex items-center gap-2"><Plus /> Post</button></div></div>
...
function PostCard({post, user}: any) {
  ...
  const handleComment = async () => {
    ...
    const newComment = { id: Date.now().toString(), userId: user.uid, userName: user.displayName, userPhoto: user.photoURL, text: commentText, replies: [], createdAt: new Date() };
  ...
  const handleReply = async (commentId: string) => {
    ...
    const updatedComments = (post.comments || []).map((c: any) => c.id === commentId? {...c, replies: [...(c.replies || []), { userId: user.uid, userName: user.displayName, text: replyText, createdAt: new Date() }]} : c);
  ...
  return (
    <div className="border rounded-xl p-4 mb-4 bg-gray-900 border-gray-800">
      ...
      <div className="mt-3 space-y-3">
        {post.comments?.map((c: any) => (<div key={c.id} className="bg-gray-800 p-3 rounded"><div className="flex gap-2"><img src={c.userPhoto} className="w-8 h-8 rounded-full"/><div className="flex-1"><b>{c.userName}:</b> {c.text}<button onClick={() => setReplyingTo(c.id)} className="text-xs text-blue-400 ml-2">Reply</button><div className="ml-4 mt-2 space-y-1">{c.replies?.map((r: any, i: number) => <div key={i} className="text-sm bg-gray-900 p-2 rounded"><b>{r.userName}:</b> {r.text}</div>)}</div>{replyingTo === c.id && <div className="flex gap-2 mt-2"><input value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Write reply..." className="flex-1 bg-gray-900 p-2 rounded text-sm" /><button onClick={() => handleReply(c.id)} className="bg-blue-500 px-3 rounded text-sm">Reply</button></div>}</div></div></div>))}
        <div className="flex gap-2"><input value={commentText} onChange={e => setCommentText(e.target.value)} placeholder="Add comment..." className="flex-1 bg-gray-800 p-2 rounded" /><button onClick={handleComment} className="bg-blue-500 px-3 rounded">Send</button></div>
      </div>
    </div>
  )
}
...
function UserCard({u, user}: any) {
  ...
  const isGoalMatch = myProfile?.goal && u.goal && myProfile.goal === u.goal; const toggleFollow = async () => { const followRef = doc(db, "follows", `${user.uid}_${u.uid}`); if(isFollowing) {await deleteDoc(followRef)} else {await setDoc(followRef, { followerId: user.uid, followingId: u.uid, createdAt: serverTimestamp() })} setIsFollowing(!isFollowing); };
...
// ===== PROFILE TAB - PIC UPLOAD + ENDORSEMENTS =====
function ProfileTab({user, onLogout}: any) {
  ...
  const [newProject, setNewProject] = useState({title: "", link: "", type: ""});
  ...
  const handleSave = async () => {
    const futureScore = (profile.growthScore || 0) + (projects.length * 10) + (profile.streak || 0) * 5;
    await setDoc(doc(db, "users", user.uid), {
      name, username, bio, goal, learning,
      skills: skills.split(",").map(s => s.trim()).filter(s => s),
      projects, futureScore
    }, { merge: true });
    setIsEditing(false);
    toast("Profile Saved!")
  };
    ...
    <div className="border rounded-xl p-6 bg-gray-900">
      <div className="flex justify-between items-start">
        <div className="relative">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="absolute bottom-0 right-0 bg-blue-500 p-1 rounded-full"><Camera size={14}/></button>
      ...
      {isEditing? (
        <div className="space-y-3 mt-4">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <input value={newProject.type} onChange={e => setNewProject({...newProject, type: e.target.value})} placeholder="Type" className="w-32 bg-gray-800 p-2 rounded" />
              <input value={newProject.link} onChange={e => setNewProject({...newProject, link: e.target.value})} placeholder="Link" className="flex-1 bg-gray-800 p-2 rounded" />
              <button onClick={() => {setProjects([...projects, {...newProject, id: Date.now()}]); setNewProject({title: "", link: "", type: ""})}} className="bg-blue-500 px-3 rounded"><Plus /></button>
            </div>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="flex gap-2 flex-wrap mt-3">
            {profile?.skills?.map((s: string) => <span key={s} className="bg-blue-500/20 px-3 py-1 rounded-full text-sm">🏷️ {s} {profile.endorsements?.[s] || 0}</span>)}
          </div>
          <div className="border-t border-gray-800 pt-4 mt-4">
            <h3 className="font-bold mb-3 flex items-center gap-2"><Rocket /> Projects</h3>
            {projects.length === 0? <p className="opacity-70">No projects yet</p> : projects.map(p => <a key={p.id} href={p.link} target="_blank" className="block hover:underline">{p.type} {p.title}</a>)}
          </div>
        </>
      )}
  ...
  return (
    <div className="flex h-[70vh] border rounded-xl bg-gray-900">
      <div className="w-1/3 border-r overflow-y-auto">
        ...
        {rooms.length === 0 && <p className="p-4 opacity-70">No chats yet</p>}
        {rooms.map(room =>
        ...
        {activeRoom?
          <>
            <div className="flex-1 p-4 overflow-y-auto">
              {messages.map(m =>
                <div key={m.id} className={`mb-2 ${m.senderId === user.uid? "text-right" : ""}`}>
                  <span className={`inline-block p-2 rounded ${m.senderId === user.uid? "bg-blue-500" : "bg-gray-800"}`}>
                    {m.text}
                  </span>
                </div>
              )}
            ...
            <div className="p-4 flex gap-2">
              <input value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} className="flex-1 bg-gray-800 p-2 rounded" />
              <button onClick={sendMessage} className="bg-blue-500 px-4 rounded">Send</button>
            </div>
          </>
          :
          <div className="flex-1 flex items-center justify-center opacity-70">Select a chat</div>
        }
      </div>
    </div>
  )
