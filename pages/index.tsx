...
import { Heart, Users, BadgeCheck, Image as ImageIcon, Mic, MicOff, Video, VideoOff, PhoneOff, Send, MessageCircle, Share2, X, Bell, UserPlus, UserCheck, MessageSquare, ArrowLeft, Headphones, Video as VideoIcon, Monitor, CircleDot, Radio, Plus, User, Grid, Sun, Moon, Download, Shield, Trash2, Ban, Megaphone, Lock } from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
import { motion, AnimatePresence } from "framer-motion";
import { loadStripe } from "@stripe/stripe-js";

const stripePromise = loadStripe("pk_test_123"); // NEE STRIPE KEY PETTU
const AGORA_APP_ID = "0e76b7daaa4b47cba8e18d1697d72";
const ADMIN_ID = "admin";
const ROOM_PRICE = 99;

const MY_USER = { id: "guest", name: "Guest", username: "guest_user", verified: true, isAdmin: false };

const initialUsers = [
  { id: "anesh", name: "Anesh", username: "anesh.dev", verified: true, followers: 120, avatar: "https://i.pravatar.cc/150?u=anesh", cover: "https://picsum.photos/seed/cover1/800/300", online: true, banned: false },
  { id: "priya", name: "Priya", username: "priya.codes", verified: false, followers: 85, avatar: "https://i.pravatar.cc/150?u=priya", cover: "https://picsum.photos/seed/cover2/800/300", online: false, banned: false },
];

const initialPosts = [
  { id: "1", userId: "guest", user: "Guest", text: "Nexora Launch! 🔥", image: "https://picsum.photos/seed/post1/400/400", likes: 12, comments: [] },
];
...
function Room({ channel, onLeave, roomType, isPaid }: { channel: string; onLeave: () => void; roomType: "voice" | "video" | "live"; isPaid: boolean }) {
  ...
  const toggleScreenShare = async () => { if(!AgoraRTC) return; if(!screenOn){ const screenTrack = await AgoraRTC.createScreenVideoTrack(); await agoraClient.unpublish(localTracks.video); await agoraClient.publish(screenTrack); setLocalTracks({...localTracks, screen: screenTrack}); screenTrack.play(localRef.current!); setScreenOn(true); toast.success("Screen Sharing Started"); } else { await agoraClient.unpublish(localTracks.screen); await agoraClient.publish(localTracks.video); localTracks.screen.close(); setLocalTracks({...localTracks, screen: null}); localTracks.video.play(localRef.current!); setScreenOn(false); } }
  ...
  return (
    <div className="relative h-screen bg-white dark:bg-black text-black dark:text-white">
      ...
      <div className="absolute bottom-0 w-full bg-gray-100 dark:bg-gray-900 p-4 flex justify-center gap-4">
        ...
        <button onClick={toggleCam} className={`p-3 rounded-full ${camOn? "bg-gray-300 dark:bg-gray-700" : "bg-red-600"}`}>{camOn? <Video /> : <VideoOff />}</button>
...
// ================== PART 3: PROFILE ==================
function ProfilePage({ user, posts, onBack }: { user: any, posts: any[], onBack: () => void }) {
  ...
  return (
    <div className="bg-white dark:bg-black text-black dark:text-white min-h-screen">
      ...
      <div className="p-4 -mt-12 relative"><div className="relative w-24 h-24 md:w-32 md:h-32"><img src={avatar} className="w-full h-full rounded-full border-4 border-white dark:border-black"/><label className="absolute bottom-0 right-0 bg-blue-600 p-2 rounded-full cursor-pointer text-white"><ImageIcon size={14}/><input type="file" accept="image/*" onChange={handleAvatarUpload} className="hidden"/></label></div>
        <div className="mt-4"><div className="flex items-center gap-2"><h1 className="text-2xl font-bold">{user.name}</h1>{user.verified && <BadgeCheck className="text-blue-500 fill-blue-500" />}</div><p className="text-gray-500 dark:text-gray-400">@{user.username}</p><div className="flex gap-6 mt-2"><p><span className="font-bold">{userPosts.length}</span> Posts</p><p><span className="font-bold">{user.followers}</span> Followers</p></div></div>
...
export default function Nexora() {
  ...
  const [currentUser, setCurrentUser] = useState({...MY_USER, isAdmin: true}); // TEST KOSAM ADMIN
  const [notifications, setNotifications] = useState<any[]>([]);
  const [chats, setChats] = useState<any[]>([{ id: "group1", type: "group", name: "Nexora Group", messages: [] }]);
  ...
  if(view === "profile") return <ProfilePage user={profileUser} posts={posts} onBack={() => setView("feed")} />;
    ...
    <div className="bg-white dark:bg-black text-black dark:text-white min-h-screen transition-colors">
      ...
      <header className="sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur p-4 flex justify-between items-center z-30 border-b border-gray-200 dark:border-gray-800">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button onClick={() => openProfile(MY_USER)} className="w-8 h-8 rounded-full"><img src="https://i.pravatar.cc/150?u=guest" className="rounded-full"/></button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-4">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-100 dark:bg-gray-900 p-4 rounded-xl"><h3 className="font-bold mb-2">Free Video</h3><button onClick={() => joinRoom("free", "free-room", "video")} className="w-full bg-blue-600 py-2 rounded-lg font-bold text-white">Join</button></div>
          ...
          <div className="bg-gray-100 dark:bg-gray-900 p-4 rounded-xl border-2 border-yellow-500"><h3 className="font-bold mb-2 flex items-center gap-1"><Lock size={16}/> Premium</h3><button onClick={() => joinRoom("paid", "paid-room", "live")} className="w-full bg-yellow-500 text-black py-2 rounded-lg font-bold">₹{ROOM_PRICE}</button></div>
        </div>
        {posts.map(post => (<div key={post.id} className="bg-gray-100 dark:bg-gray-900 p-4 rounded-xl mb-4"><div onClick={() => openProfile(users.find(u=>u.id===post.userId) || MY_USER)} className="font-bold cursor-pointer">@{post.user}</div><p className="my-2">{post.text}</p><img src={post.image} className="rounded-lg w-full"/></div>))}
      </main>
    </div>
  );
}
