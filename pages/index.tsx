...
import { Heart, MessageCircle, Send, LogIn, Sun, Moon, X, Image as ImageIcon } from "lucide-react";
...
export default function NexoraLite() {
  ...
  if(!user) return (
    <div className="h-screen flex flex-col items-center justify-center bg-black text-white">
      <Toaster />
      <h1 className="text-5xl font-bold mb-2 bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent">Nexora</h1>
      <p className="mb-8 text-gray-400">Simple Social Media</p>
      <button onClick={login} className="bg-white text-black px-6 py-3 rounded-lg font-bold flex items-center gap-2 hover:scale-105 transition"><LogIn /> Continue with Google</button>
    </div>
  )
      ...
      <main className="max-w-lg mx-auto pb-20">
          ...
          {[1,2,3,4,5].map((i) => (
            <div key={i} className="flex flex-col items-center gap-1 flex-shrink-0">
              <div className="w-16 h-16 rounded-full p-[2px] bg-gradient-to-tr from-yellow-400 to-pink-500">
                <img src={`https://i.pravatar.cc/150?u=${i}`} className="w-full h-full rounded-full border-2 border-white dark:border-black"/>
              </div>
        ...
        <div className="p-4 border-b dark:border-gray-800">
          <div className="flex gap-3 mb-3">
            ...
            <input value={newPostText} onChange={e=>setNewPostText(e.target.value)} placeholder="What's on your mind?" className="w-full bg-gray-100 dark:bg-gray-900 p-2 rounded-lg outline-none"/>
          </div>
          <label className="flex items-center gap-2 mb-3 cursor-pointer text-gray-500"><ImageIcon size={18}/> <span>Upload Photo</span><input type="file" accept="image/*" onChange={e=>setNewPostFile(e.target.files?.[0] || null)} className="hidden"/></label>
          {newPostFile && <p className="text-sm mb-2">{newPostFile.name}</p>}
