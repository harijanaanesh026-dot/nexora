...
const COLLEGES_DATA:any = {
  "BITS": { domains:["bits"], example:"BITS2021001" },
  "SRET": { domains:["sret"], example:"SRET21001" },
  "SVCE": { domains:["svce"], example:"SVCE21001" },
  "ST.JOHNS": { domains:["stjohns","johns"], example:"SJ21001" },
  "ARTS & SCIENCE": { domains:["arts"], example:"ARTS21001" },
  "VEMU": { domains:["vemu"], example:"VEMU21001" },
  "OTHER": { domains:[], example:"OTHER001" },
};
const TOPICS = [
...
export default function YakApp(){
  ...
  const handleLike=async(y:any, type:'like'|'dislike')=>{
    ...
    }catch(e:any){ console.log(e); alert(e.message); }
  };

  // ✅ FIXED POST - NO BASE64 BIG ISSUE
  const handlePost=async()=>{
    if(!user ||!userData?.id) return alert('Login ayyaledu - refresh chey bro!');
    if(!newYak.trim() && images.length===0 &&!poll.q1.trim()) return alert('Emanna rayi bro! Text / Photo / Poll');
    if(newYak.length>500) return alert('500 chars max bro!');
    setPosting(true);
    try{
      console.log('Posting...', {college:userData.college, text:newYak});
      const payload:any={
        text:newYak.trim(),
        uid:user.uid,
        ...
        imageUrls: images.slice(0,2), // max 2 to avoid Firestore 1MB limit
        poll: poll.q1.trim()?{q1:poll.q1.trim(),q2:poll.q2.trim()||'No',v1:0,v2:0,voters:[]}:null,
        createdAt:serverTimestamp()
      };
      const docRef=await addDoc(collection(db,'yaks'),payload);
      ...
      alert('Post failed: '+e.message+'\n\nFirestore Rules check chey bro!');
    ...
    return(
      <div className="min-h-screen bg-[#0a0a0a] text-white p-6 flex flex-col items-center justify-center">
        <div className="w-full max-w-md">
          ...
          <div className="flex gap-2 mt-6 p-1.5 bg-[#141414] border border-zinc-800 rounded-full w-fit">{[{id:'email',l:'📧 Email'},{id:'roll',l:'🎫 Roll'},{id:'idcard',l:'🪪 ID Card'}].map(m=><button key={m.id} onClick={()=>setVerifyMethod(m.id as any)} className={`px-4 py-2 rounded-full text-xs font-bold ${verifyMethod===m.id?'bg-white text-black':'text-zinc-500'}`}>{m.l}</button>)}</div>
          {verifyMethod==='email' && <div className="mt-6 bg-[#141414] border border-zinc-800 rounded-[24px] p-5"><p className="font-bold text-sm">College Email Verify <span className="ml-2 text-[10px] bg-green-500/20 text-green-400 px-2 py-1 rounded-full">Instant</span></p><input value={collegeEmail} onChange={e=>setCollegeEmail(e.target.value)} placeholder={`yourname@${cd?.domains[0]||'college'}.edu.in`} className="w-full mt-4 p-4 bg-[#0a0a0a] border border-zinc-800 rounded-2xl text-sm outline-none"/><!otpSent? <button onClick={handleEmailVerify} className="w-full mt-4 bg-white text-black py-4 rounded-full font-bold">Send OTP →</button> : <div className="mt-4"><input value={otp} onChange={e=>setOtp(e.target.value)} placeholder="Enter 6-digit OTP" className="w-full p-4 bg-[#0a0a0a] border border-zinc-800 rounded-2xl text-center tracking-widest"/><button onClick={handleOtpSubmit} className="w-full mt-3 bg-yellow-400 text-black py-4 rounded-full font-bold">Verify OTP ✓</button><p className="text-[10px] text-zinc-500 text-center mt-2">Demo OTP: {generatedOtp}</p></div>}</div>}
          {verifyMethod==='roll' && <div className="mt-6 bg-[#141414] border border-zinc-800 rounded-[24px] p-5"><p className="font-bold text-sm">Roll Number Verify</p><p className="text-[11px] text-zinc-500 mt-1">Ex: {cd?.example}</p><input value={rollNumber} onChange={e=>setRollNumber(e.target.value)} placeholder={cd?.example} className="w-full mt-4 p-4 bg-[#0a0a0a] border border-zinc-800 rounded-2xl text-sm uppercase"/><button onClick={handleRollVerify} className="w-full mt-4 bg-white text-black py-4 rounded-full font-bold">Verify Roll No →</button></div>}
          {verifyMethod==='idcard' && <div className="mt-6 bg-[#141414] border border-zinc-800 rounded-[24px] p-5"><p className="font-bold text-sm">ID Card Upload</p><label className="w-full mt-4 border-2 border-dashed border-zinc-800 p-6 rounded-2xl flex flex-col items-center cursor-pointer">{idCardImage? <img src={idCardImage} className="h-32 rounded-xl"/> : <><span className="text-3xl">📸</span><p className="text-sm text-zinc-400 mt-2">Upload ID Card</p></>}<input type="file" hidden accept="image/*" onChange={e=>{ const f=e.target.files?.[0]; if(f){ const r=new FileReader(); r.onloadend=()=>setIdCardImage(r.result as string); r.readAsDataURL(f); } }}/></label><button onClick={handleIdCardVerify} disabled={!idCardImage} className={`w-full mt-4 py-4 rounded-full font-bold ${idCardImage?'bg-white text-black':'bg-zinc-800 text-zinc-500'}`}>Submit →</button></div>}
        </div>
      </div>
    );
  }
  ...
  if(screen==='pending'){
    return <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6 text-center"><div className="w-20 h-20 bg-orange-500/20 rounded-full flex items-center justify-center text-3xl">⏳</div><h1 className="text-3xl font-black mt-6">Pending Approval</h1><p className="text-zinc-500 text-sm mt-2">Admin will approve {userData?.college} ID</p><button onClick={()=>{auth.signOut(); localStorage.clear(); window.location.reload();}} className="mt-6 bg-zinc-800 px-6 py-3 rounded-full text-sm">Logout</button></div>;
            }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
          ...
          return(
            <div key={y.id} className="bg-[#141414] border border-zinc-800 rounded-[28px] p-5 hover:border-zinc-700 transition">
              <div className="flex justify-between items-start">
                <div className="flex gap-3 items-center"><div className="w-10 h-10 bg-[#1f1f1f] border border-zinc-800 rounded-full flex items-center justify-center text-lg">{y.avatar||'👻'}</div><div><p className="text-[13px] font-black flex items-center gap-2">{y.username} {isOwner && <span className="bg-yellow-400 text-black px-2 py-0.5 rounded-full text-[9px]">YOU</span>} <span className={`text-[9px] px-2.5 py-1 rounded-full ${TOPICS.find(t=>t.name===y.topic)?.color||'bg-zinc-700'} text-white font-bold`}>{y.topic}</span></p><p className="text-[11px] text-zinc-500">{y.college} • {y.createdAt?.toDate? new Date(y.createdAt.toDate()).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'now'} • {score>0?`+${score}`:score}</p></div></div>
                {isOwner && <button onClick={async()=>{ if(confirm('Delete this yak?')){ try{ await deleteDoc(doc(db,'yaks',y.id)); }catch(e:any){ alert(e.message); } } }} className="w-8 h-8 bg-red-500/10 border border-red-500/20 rounded-full text-xs hover:bg-red-500/20">🗑️</button>}
              </div>
              {y.text && <p className="mt-4 text-[16.5px] leading-[1.6] whitespace-pre-wrap">{y.text}</p>}
              ...
              {y.poll && <div className="mt-4 space-y-2">{[{q:y.poll.q1,v:y.poll.v1},{q:y.poll.q2,v:y.poll.v2}].map((opt,idx)=><button key={idx} onClick={async()=>{ if(y.poll.voters?.includes(user.uid)) return alert('Already voted'); await updateDoc(doc(db,'yaks',y.id),{[idx===0?'poll.v1':'poll.v2']:increment(1),'poll.voters':arrayUnion(user.uid)}); }} className="w-full bg-[#1f1f1f] border border-zinc-800 p-4 rounded-2xl text-left relative overflow-hidden"><div className="absolute inset-0 bg-yellow-400/10" style={{width:`${total? (opt.v/total)*100:0}%`}}></div><div className="relative flex justify-between"><p className="font-bold text-sm">{opt.q}</p><span className="bg-zinc-800 px-3 py-1 rounded-full text-xs font-black">{opt.v||0}</span></div></button>)}<p className="text-[11px] text-zinc-500">{total} votes • {y.college} only</p></div>}
              ...
              <div className="flex items-center gap-2 mt-5">
                ...
                <div className="ml-auto"><span className={`px-3 py-1.5 rounded-full text-[11px] font-black border ${liked?'bg-white text-black border-white':disliked?'bg-red-500/20 text-red-400 border-red-500/20':'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>{liked?'❤️ You liked':disliked?'💔 You disliked':'✨ New'}</span></div>
              </div>
              {(y.likes||0)+(y.dislikes||0)>0 && <div className="mt-3 h-1 bg-[#1f1f1f] rounded-full overflow-hidden flex"><div className="h-full bg-white" style={{width:`${((y.likes||0)/((y.likes||0)+(y.dislikes||0)))*100}%`}}></div><div className="h-full bg-red-500/60" style={{width:`${((y.dislikes||0)/((y.likes||0)+(y.dislikes||0)))*100}%`}}></div></div>}
              {activePost===y.id && <div className="mt-4 pt-4 border-t border-zinc-800"><div className="space-y-3 max-h-64 overflow-y-auto">{comments.map(c=><div key={c.id} className="flex gap-2.5"><div className="w-8 h-8 bg-[#1f1f1f] border border-zinc-800 rounded-full flex items-center justify-center text-sm shrink-0">{c.avatar||'👻'}</div><div className="bg-[#1f1f1f] border border-zinc-800 rounded-2xl px-4 py-2.5 flex-1"><p className="text-[13px]">{c.text}</p><p className="text-[10px] text-zinc-500 mt-1">{c.username}</p></div><button onClick={async()=>{ await deleteDoc(doc(db,`yaks/${y.id}/comments/${c.id}`)); await updateDoc(doc(db,'yaks',y.id),{commentsCount:increment(-1)}); }} className="text-[11px]">✕</button></div>)}{comments.length===0 && <p className="text-xs text-zinc-600 text-center py-4">No comments yet</p>}</div><div className="flex gap-2 mt-4"><div className="w-8 h-8 bg-[#1f1f1f] border border-zinc-800 rounded-full flex items-center justify-center shrink-0">{userData.avatar}</div><div className="flex-1 relative"><input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Anonymous comment..." className="w-full bg-[#1f1f1f] border border-zinc-800 rounded-full pl-4 pr-12 py-3 text-sm outline-none"/><button onClick={async()=>{ if(!commentText.trim()) return; await addDoc(collection(db,`yaks/${y.id}/comments`),{text:commentText,uid:user.uid,username:userData.username,avatar:userData.avatar,createdAt:serverTimestamp()}); await updateDoc(doc(db,'yaks',y.id),{commentsCount:increment(1)}); setCommentText(''); }} className="absolute right-1 top-1 bg-yellow-400 text-black w-9 h-9 rounded-full font-black">↑</button></div></div></div>}
            </div>
          )
        })}
      ...
      {/* ✅ CREATE SCREEN - POST FIXED */}
      {screen==='create' && (
        <div className="fixed inset-0 bg-[#0a0a0a] z-30 p-4 overflow-y-auto">
          <div className="max-w-xl mx-auto">
            <div className="flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-[#1f1f1f] border border-zinc-800 rounded-full flex items-center justify-center">{userData.avatar}</div><div><h2 className="font-black">New Yak</h2><p className="text-xs text-zinc-500">{userData.college} • Verified {userData.verifyMethod} 🔒</p></div></div><button onClick={()=>{setScreen('feed'); setNewYak(''); setImages([]); setPoll({q1:'',q2:''});}} className="w-10 h-10 bg-[#1a1a1a] border border-zinc-800 rounded-full">✕</button></div>
            <div className="flex gap-2 mt-6 overflow-x-auto scrollbar-hide">{TOPICS.map(t=><button key={t.name} onClick={()=>setTopic(t.name)} className={`px-4 py-2.5 rounded-full text-xs font-bold border whitespace-nowrap ${topic===t.name?'bg-yellow-400 text-black':'bg-[#1a1a1a] border-zinc-800 text-zinc-500'}`}>{t.icon} {t.name}</button>)}</div>
            <textarea value={newYak} onChange={e=>setNewYak(e.target.value)} placeholder={`What's happening in ${userData.college}? Be anonymous ${userData.avatar}...`} className="w-full h-36 mt-6 p-5 bg-[#141414] border border-zinc-800 rounded-[24px] outline-none text-[16px] resize-none" maxLength={500}/>
            <div className="flex justify-between mt-2"><p className="text-[11px] text-zinc-500">{newYak.length}/500</p><p className="text-[11px] text-zinc-500">Topic: {topic}</p></div>
            ...
            <label className="w-full mt-4 border-2 border-dashed border-zinc-800 p-5 rounded-[24px] flex flex-col items-center text-sm text-zinc-500 bg-[#141414] cursor-pointer hover:border-yellow-400/30">🖼️ Add Photos (max 2, optional)<input type="file" multiple hidden accept="image/*" onChange={e=>{ const files=Array.from(e.target.files||[]).slice(0,2); files.forEach((f:any)=>{ const r=new FileReader(); r.onloadend=()=>setImages(p=>[...p,r.result as string].slice(0,2)); r.readAsDataURL(f); }); }}/></label>
            {images.length>0 && <div className="grid grid-cols-2 gap-2 mt-3">{images.map((im,i)=><div key={i} className="relative"><img src={im} className="h-32 rounded-xl object-cover w-full border border-zinc-800"/><button onClick={()=>setImages(images.filter((_,idx)=>idx!==i))} className="absolute -top-2 -right-2 bg-red-500 w-7 h-7 rounded-full text-xs font-bold">x</button></div>)}</div>}
            <button onClick={handlePost} disabled={posting || (!newYak.trim() && images.length===0 &&!poll.q1.trim())} className={`w-full mt-8 p-4 rounded-full font-black text-lg transition-all ${posting || (!newYak.trim() && images.length===0 &&!poll.q1.trim())?'bg-zinc-800 text-zinc-500':'bg-white text-black hover:scale-[1.02] shadow-xl'}`}>{posting?'Posting... ⏳':`Post to ${userData.college} 🚀`}</button>
            <p className="text-[11px] text-zinc-600 text-center mt-3">🔒 Only {userData.college} students will see this • {userData.verifyMethod} verified</p>
            {posting && <p className="text-[11px] text-yellow-400 text-center mt-2">Posting yak... please wait</p>}
          </div>
        </div>
      )}
      ...
      {showProfile && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-30 flex items-end sm:items-center justify-center p-4">
          <div className="bg-[#141414] border border-zinc-800 w-full sm:max-w-sm rounded-t-[32px] sm:rounded-[32px] p-6">
            ...
            <div className="mt-6 bg-green-500/10 border border-green-500/20 rounded-2xl p-4"><p className="text-xs font-bold text-green-400">✓ Verified {userData.college} Student</p><p className="text-[11px] text-zinc-400 mt-1">Karma: {userData.karma} • Isolated feed 🔒 • 1 like/dislike per user</p></div>
