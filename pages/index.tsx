...
// SHARE 3in1 - ONLY ADDITION - NEE CODE LO EM MARCHALEDU
  const handleSharePost = async (yy:any) => {
    try{
      const url = `${window.location.origin}/?post=${yy.id}`;
      if((navigator as any).share){ try{ await (navigator as any).share({title:"SRET ONLY", text:yy.text, url}); showToast("Shared 🔗"); return; }catch{} }
      await navigator.clipboard.writeText(url); showToast("Link copied 🔗?post=ID");
    }catch{}
  };
  const handleWhatsAppShare = (yy:any) => { try{ const t=encodeURIComponent(yy.text+"\n"+window.location.origin+"/?post="+yy.id); window.open(`https://wa.me/?text=${t}`,"_blank"); }catch{} };
  const handleCopyLink = async (yy:any) => { try{ const url=`${window.location.origin}/?post=${yy.id}`; await navigator.clipboard.writeText(url); showToast("Copied 🔗?post=ID"); }catch{} };
