...
const Footer = () => (
  <div className="w-full py-8 flex flex-col items-center gap-2 border-t border-zinc-900/50 mt-8 bg-[#050507]">
    ...
    <p className="text-[10px] text-zinc-600">Real Mail • Real Roll • Real ID • Attractive • Real Count</p>
  </div>
);
...
export default function YakApp(){
      ...
      if(u){
        ...
        if(snap.empty){
          ...
          await addDoc(collection(db,'users'),{
            ...
            college:String(col), collegeEmail:String(localStorage.getItem('college_email')||''), rollNumber:String(localStorage.getItem('roll_number')||''), idImage:String(localStorage.getItem('id_image')||''), idName:String(localStorage.getItem('id_name')||''), verifyMethod:String(localStorage.getItem('verify_method')||'email'),
  ...
  const handleRollVerify=async()=>{
    ...
    if(!config.rollPattern.test(rollUpper)){ setVerifyError(`❌ Invalid ${selectedCollege} roll. Example: ${config.rollExample}`); return; }
  ...
  const handleIdVerify=async()=>{
    setVerifyError(''); if(!idImage){ setVerifyError('❌ Upload real ID card photo'); return; } if(!idName.trim()){ setVerifyError('❌ Enter name as on ID'); return; }
    ...
    return(
      <div className="min-h-screen bg-[#050507] text-white flex flex-col">
        ...
        <div className="flex-1 p-6 max-w-md mx-auto w-full">
          <div className="mt-6 flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-12 h-12 bg-white rounded-[14px] flex items-center justify-center font-black text-black text-xl">Y</div><div><p className="font-black text-[18px] tracking-tighter leading-none">YAK</p><p className="text-[10px] tracking-[0.2em] text-zinc-500 font-bold">REAL VERIFIED ONLY</p></div></div><div className="bg-white/10 border border-white/20 rounded-full px-3 py-1.5 flex items-center gap-1.5"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><p className="text-[11px] font-black">{totalUsers} REAL</p></div></div>
          <h1 className="text-[42px] font-black mt-8 leading-[0.85] tracking-tighter">Real mail.<br/>Real roll.<br/><span className="text-zinc-600">Real ID.</span></h1>
          <div className="mt-4 bg-green-500/10 border border-green-500/20 rounded-[18px] p-4"><p className="text-[11px] font-black tracking-widest text-green-400">✓ 100% REAL STUDENTS ONLY</p><p className="text-[12px] text-zinc-400 mt-1">Domain + Roll pattern + ID photo verified</p></div>
  ...
  if(screen==='verify'){
    ...
    return(
      <div className="min-h-screen bg-[#050507] text-white"><div className="max-w-md mx-auto min-h-screen flex flex-col"><div className="p-6"><button onClick={()=>setScreen('college')} className="w-10 h-10 bg-zinc-900 rounded-full">←</button>
      <div className="mt-6 rounded-[28px] bg-[#0a0a0f] border border-zinc-900 p-6"><div className="flex gap-4"><div className={`w-16 h-16 rounded-[20px] ${selectedAvatar.bg} flex items-center justify-center text-3xl ${selectedAvatar.glow}`}>{selectedAvatar.emoji}</div><div className="flex-1"><p className="text-[10px] tracking-[0.2em] text-zinc-500 font-bold">REAL VERIFY • {count} REAL IN {selectedCollege}</p><h1 className="text-[20px] font-black leading-none mt-1">Prove Real Student</h1><p className="text-[11px] text-zinc-500 mt-1">Domain: {config?.domains.join(', ')}</p></div></div></div></div>
      ...
      {verifyMethod==='id' && <div className="p-6 flex-1"><div className="bg-[#0a0a0f] border border-zinc-900 rounded-[24px] p-5"><p className="text-[11px] font-black tracking-widest text-zinc-500">REAL ID CARD PHOTO</p><input value={idName} onChange={e=>setIdName(e.target.value.toUpperCase())} placeholder="NAME AS ON ID" className="w-full mt-3 p-4 bg-black border-2 border-zinc-900 rounded-2xl uppercase font-black tracking-widest outline-none"/><label className="mt-3 border-2 border-dashed border-zinc-700 rounded-[18px] p-6 flex flex-col items-center cursor-pointer"><span className="text-[11px] font-black tracking-widest text-zinc-500">📸 UPLOAD REAL ID CARD</span><input type="file" hidden accept="image/*" onChange={e=>{ const f=e.target.files?.[0]; if(f){ const r=new FileReader(); r.onloadend=()=>setIdImage(r.result as string); r.readAsDataURL(f); } }}/></label>{idImage && <div className="mt-4 relative"><img src={idImage} alt="" className="rounded-[18px] w-full border-2 border-green-500/30"/><button onClick={()=>setIdImage('')} className="absolute top-3 right-3 w-8 h-8 bg-black/80 rounded-full text-white">✕</button></div>}<button onClick={handleIdVerify} disabled={!idImage ||!idName.trim()} className={`w-full mt-4 py-4 rounded-full font-black ${!idImage||!idName.trim()?'bg-zinc-900 text-zinc-700':'bg-white text-black'}`}>Verify Real ID ✓</button></div></div>}
      <Footer/></div></div>
    );
  }
  if(screen==='login'){ const count=collegeCounts[selectedCollege]||0; return <div className="min-h-screen bg-[#050507] text-white flex flex-col items-center justify-center p-6"><div className={`w-28 h-28 rounded-[32px] border-2 border-white/10 flex items-center justify-center text-5xl ${selectedAvatar.bg} ${selectedAvatar.glow}`}>{selectedAvatar.emoji}</div><h1 className="text-[28px] font-black mt-6 tracking-tighter text-center">Real Verified ✓<br/>{selectedCollege} Herd 🔥</h1><p className="text-zinc-500 text-sm mt-2">{count} real • You #{count+1}</p><button onClick={handleGoogleLogin} className="w-full max-w-sm mt-8 bg-white text-black py-4 rounded-full font-black">Continue → Real Herd</button><div className="mt-16 w-full max-w-md"><Footer/></div></div>; }

    return(
    <div className="min-h-screen bg-[#050507] text-white flex flex-col">
      <style>{`body{background:#050507} ::-webkit-scrollbar{display:none} button{transition:all 0.25s cubic-bezier(0.16,1,0.3,1)} button:active{transform:scale(0.94)}.attractive-glow{animation:glowPulse 2.5s infinite} @keyframes glowPulse{0%,100%{filter:brightness(1) drop-shadow(0 0 12px rgba(255,255,255,0.2))}50%{filter:brightness(1.2) drop-shadow(0 0 22px rgba(255,255,255,0.35))}}.shimmer{background:linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent); background-size:200% 100%; animation:shimmer 2s infinite} @keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}`}</style>
      <div className="sticky top-0 z-30 bg-[#050507]/80 backdrop-blur-xl border-b border-zinc-900/50">
        <div className="max-w-[600px] mx-auto px-5 h-[68px] flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center font-black text-black attractive-glow">Y</div><div><p className="font-black text-[15px] tracking-tighter leading-none">{userData.college} Herd • {collegeCounts[userData.college]||0} real • {yaks.length} yaks</p><p className="text-[11px] text-zinc-500 font-bold">Real • Attractive • Instant</p></div></div>
          <button onClick={()=>setShowProfile(true)} className={`w-11 h-11 rounded-full ${userData.avatarBg} border-2 border-white/20 flex items-center justify-center text-xl ${userData.avatarGlow} attractive-glow`}>{userData.avatar}</button>
        </div>
      </div>

      <div className="max-w-[600px] mx-auto w-full flex-1 pb-[90px] p-3 space-y-3 mt-2">
        <div className="relative bg-gradient-to-br from-white via-zinc-100 to-zinc-300 rounded-[28px] p-[1.5px] attractive-glow">
          <div className="bg-gradient-to-br from-white to-zinc-200 rounded-[26px] p-5 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-violet-500/20 to-cyan-500/20 rounded-full blur-[30px]"></div>
            <div className="relative flex items-start justify-between">
              <div className="flex gap-4"><div className="relative"><div className={`w-[72px] h-[72px] rounded-[20px] ${userData.avatarBg} border-[3px] border-black/10 flex items-center justify-center text-[32px] ${userData.avatarGlow} attractive-glow`}>{userData.avatar}</div><div className="absolute -bottom-1 -right-1 w-7 h-7 bg-green-500 border-[3px] border-white rounded-full flex items-center justify-center text-[12px] font-black">✓</div></div>
                <div><div className="flex items-center gap-2"><h2 className="font-black text-[20px] tracking-tighter leading-none">{userData.username}</h2><span className="px-2.5 py-1 bg-black text-white rounded-full text-[9px] font-black tracking-widest">ATTRACTIVE</span></div><p className="text-[12px] font-bold opacity-70 mt-1.5">{userData.college} herd • {collegeCounts[userData.college]||0} real • Verified ✓</p><div className="flex gap-2 mt-3"><div className="px-3.5 py-2 bg-black text-white rounded-full text-[11px] font-black flex items-center gap-1.5">🔥 {userData.yakarma} Yakarma</div><div className="px-3 py-2 bg-green-500 text-black rounded-full text-[10px] font-black">✓ REAL • #{totalUsers}</div></div></div>
              </div>
              <div className="text-right"><p className="text-[28px] font-black leading-none">{collegeCounts[userData.college]||0}</p><p className="text-[9px] font-black tracking-[0.15em] opacity-60">REAL IN HERD</p></div>
            </div>
            <div className="mt-5 grid grid-cols-4 gap-2"><div className="bg-black/5 border border-black/10 rounded-[14px] p-2.5 text-center"><p className="text-[16px] font-black">{userData.totalPosts||0}</p><p className="text-[8px] font-bold tracking-widest opacity-60">YAKS</p></div><div className="bg-black/5 border border-black/10 rounded-[14px] p-2.5 text-center"><p className="text-[16px] font-black">{collegeCounts[userData.college]||0}</p><p className="text-[8px] font-bold tracking-widest opacity-60">REAL</p></div><div className="bg-black/5 border border-black/10 rounded-[14px] p-2.5 text-center"><p className="text-[16px] font-black">{yaks.length}</p><p className="text-[8px] font-bold tracking-widest opacity-60">FEED</p></div><div className="bg-black text-white border border-black rounded-[14px] p-2.5 text-center"><p className="text-[16px] font-black">{userData.yakarma}</p><p className="text-[8px] font-bold tracking-widest opacity-80">KARMA ✨</p></div></div>
          </div>
        </div>
        ...
        {yaks.map(y=>{
          ...
          return(
            <div key={y.id} className={`bg-[#0a0a0f] border rounded-[26px] p-5 hover:border-zinc-700 transition-all relative ${isOwn?'border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.06)]':'border-zinc-900'}`}>
              <div className="flex justify-between items-start"><div className="flex gap-3"><div className={`w-11 h-11 rounded-full ${y.avatarBg||'bg-zinc-900'} border-2 flex items-center justify-center text-lg relative ${y.avatarGlow||''} shrink-0 ${isOwn?'border-white/30 attractive-glow':'border-zinc-800'}`}>{y.avatar||'👻'}{isOwn && <div className="absolute -top-1 -right-1 w-4 h-4 bg-white text-black rounded-full flex items-center justify-center text-[8px] font-black">YOU</div>}<div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-[#0a0a0f] flex items-center justify-center text-[8px]">✓</div></div><div><div className="flex items-center gap-2 flex-wrap"><p className={`font-black text-[13px] ${isOwn?'text-white':''}`}>{y.username} {isOwn&&'• YOU ✨'}</p><span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black ${isOwn?'bg-white text-black attractive-glow':'bg-white/10 text-zinc-400 border border-white/10'}`}>{y.college} • {collegeCounts[y.college]||0} REAL</span>{y.edited&&<span className="text-[9px] text-zinc-500">(edited)</span>}</div><p className="text-[10px] text-zinc-500">{isOwn?'Your attractive yak •':'Attractive herd •'} {y.college}</p></div></div>
                {isOwn && (<div className="relative"><button onClick={()=>setShowMenu(showMenu===y.id?null:y.id)} className="w-8 h-8 bg-[#111113] border border-zinc-800 rounded-full flex items-center justify-center text-zinc-500 hover:text-white">⋯</button>{showMenu===y.id && (<div className="absolute right-0 top-10 w-[170px] bg-[#111113] border border-zinc-800 rounded-[18px] p-2 z-20 shadow-[0_20px_50px_rgba(0,0,0,0.7)]"><button onClick={()=>{ setEditingPost(y); setEditText(y.text); setShowMenu(null); }} className="w-full text-left px-3.5 py-3 rounded-[12px] text-[13px] font-bold hover:bg-white hover:text-black flex items-center gap-2"><span>✏️</span> Edit Yak</button><button onClick={()=>handleDelete(y)} className="w-full text-left px-3.5 py-3 rounded-[12px] text-[13px] font-bold hover:bg-red-500 hover:text-white text-red-400 flex items-center gap-2 mt-1"><span>🗑️</span> Delete Yak</button><button onClick={()=>setShowMenu(null)} className="w-full text-left px-3 py-2 rounded-xl text-[11px] font-bold text-zinc-500 hover:bg-zinc-800 mt-2">Cancel</button></div>)}</div>)}
              </div>
        ...
        {yaks.length===0 && <div className="py-16 text-center"><div className={`w-28 h-28 rounded-[32px] mx-auto flex items-center justify-center text-5xl border-[3px] border-white/20 ${userData.avatarBg} ${userData.avatarGlow} attractive-glow`}><span>{userData.avatar}</span></div><p className="font-black mt-6 text-[22px] tracking-tighter">Welcome {userData.username} ✨</p><p className="text-[13px] font-bold text-white/60 mt-1">Attractive • {userData.college} • {collegeCounts[userData.college]||0} real</p><button onClick={()=>setScreen('create')} className="mt-6 bg-white text-black px-8 h-12 rounded-full font-black text-sm attractive-glow">+ First Attractive Yak in {userData.college}</button><div className="mt-12"><Footer/></div></div>}
        {yaks.length>0 && <Footer/>}
      </div>

      <div className="fixed bottom-0 left-0 right-0 bg-[#050507]/80 backdrop-blur-xl border-t border-zinc-900/50"><div className="max-w-[600px] mx-auto px-8 h-[80px] flex items-center justify-between"><button className="flex flex-col items-center gap-1 text-white"><div className="w-7 h-7 bg-white rounded-full flex items-center justify-center text-black font-black text-xs attractive-glow">⌂</div><span className="text-[9px] font-black tracking-widest">{userData.college} • {collegeCounts[userData.college]||0} REAL</span></button><button onClick={()=>setScreen('create')} className="w-14 h-14 bg-white text-black rounded-full flex items-center justify-center text-2xl shadow-[0_0_30px_rgba(255,255,255,0.3)] attractive-glow">+</button><button onClick={()=>setShowProfile(true)} className="flex flex-col items-center gap-1"><div className={`w-7 h-7 rounded-full ${userData.avatarBg} border border-white/20 flex items-center justify-center text-sm ${userData.avatarGlow} attractive-glow`}>{userData.avatar}</div><span className="text-[9px] font-black tracking-widest">YOU ✨ • {userData.yakarma}</span></button></div></div>

      {screen==='create' && (
        <div className="fixed inset-0 bg-[#050507] z-40"><div className="max-w-[600px] mx-auto h-full flex flex-col"><div className="p-4 flex items-center justify-between border-b border-zinc-900/50"><button onClick={()=>setScreen('feed')} className="w-9 h-9 bg-zinc-900 rounded-full text-white">✕</button><div className="flex items-center gap-2"><div className={`w-7 h-7 rounded-full ${userData.avatarBg} flex items-center justify-center text-sm ${userData.avatarGlow}`}>{userData.avatar}</div><p className="font-bold text-[11px] tracking-widest">{userData.college} • ATTRACTIVE • {collegeCounts[userData.college]||0} REAL</p></div><button onClick={handlePost} disabled={posting ||!newYak.trim()} className={`px-5 h-9 rounded-full font-black text-sm ${!newYak.trim()?'bg-zinc-900 text-zinc-700':'bg-white text-black attractive-glow'}`}>{posting?'Posting...':'Yak ✨'}</button></div>
        <div className="p-5 flex-1 overflow-y-auto"><textarea value={newYak} onChange={e=>setNewYak(e.target.value)} placeholder={`What's happening in ${userData.college}? Attractive ga cheppu... ${collegeCounts[userData.college]||0} real students chusthunnaru ✨`} autoFocus className="w-full bg-transparent text-[22px] outline-none placeholder:text-zinc-800 resize-none min-h-[160px]" maxLength={300}/><label className="mt-6 border-2 border-dashed border-zinc-800 rounded-[18px] p-6 flex flex-col items-center cursor-pointer hover:border-white/30"><span className="text-[11px] font-black tracking-widest text-zinc-500">ADD ATTRACTIVE PHOTO ✨</span><input type="file" hidden accept="image/*" onChange={e=>{ const f=e.target.files?.[0]; if(f){ const r=new FileReader(); r.onloadend=()=>setImages([r.result as string]); r.readAsDataURL(f); } }}/></label>{images[0] && <div className="mt-4 relative"><img src={images[0]} alt="" className="rounded-[18px] w-full border border-zinc-900"/><button onClick={()=>setImages([])} className="absolute top-3 right-3 w-8 h-8 bg-black/80 rounded-full text-white">✕</button></div>}</div><div className="p-4 border-t border-zinc-900/50"><Footer/></div></div></div>
      )}

      {editingPost && (<div className="fixed inset-0 bg-black/70 backdrop-blur-xl z-[60] flex items-end justify-center p-3"><div className="bg-[#0a0a0f] border border-zinc-800 w-full max-w-[600px] rounded-t-[28px] p-6 pb-8"><div className="w-10 h-1.5 bg-zinc-800 rounded-full mx-auto mb-6"></div><div className="flex items-center justify-between"><h2 className="font-black text-[18px]">✏️ Edit Attractive Yak</h2><span className="text-[10px] bg-white text-black px-2 py-1 rounded-full font-black">{editingPost.college} • {collegeCounts[editingPost.college]||0} real</span></div><textarea value={editText} onChange={e=>setEditText(e.target.value)} className="w-full mt-4 bg-[#111113] border border-zinc-800 rounded-[18px] p-4 text-[16px] outline-none focus:border-white/30 min-h-[120px] resize-none" autoFocus/><div className="flex gap-3 mt-5"><button onClick={()=>{ setEditingPost(null); setEditText(''); }} className="flex-1 h-12 bg-zinc-900 border border-zinc-800 rounded-full font-bold">Cancel</button><button onClick={handleEdit} disabled={!editText.trim()} className={`flex-1 h-12 rounded-full font-black ${!editText.trim()?'bg-zinc-800 text-zinc-600':'bg-white text-black attractive-glow'}`}>Save Attractive ✓</button></div></div></div>)}
      {showProfile && (
        ...
        <div className="relative bg-gradient-to-br from-white via-zinc-100 to-zinc-200 rounded-[32px] p-[1.5px] attractive-glow"><div className="bg-gradient-to-br from-zinc-900 via-black to-zinc-900 rounded-[30px] p-6 relative overflow-hidden"><div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-violet-500/20 to-cyan-500/15 rounded-full blur-[35px]"></div><div className="relative flex gap-5"><div className="relative"><div className={`w-24 h-24 rounded-[26px] ${userData.avatarBg} border-[3px] border-white/20 flex items-center justify-center text-[38px] ${userData.avatarGlow} attractive-glow`}>{userData.avatar}</div><div className="absolute -bottom-2 -right-2 w-8 h-8 bg-green-500 border-[3px] border-[#0a0a0f] rounded-full flex items-center justify-center text-[14px]">✓</div></div><div className="flex-1"><h2 className="font-black text-[24px] tracking-tighter leading-none text-white">{userData.username} • Attractive ✨</h2><p className="text-[13px] font-bold text-white/60 mt-1.5">{userData.college} • {collegeCounts[userData.college]||0} real verified • Real ID ✓</p><div className="flex gap-2 mt-4"><span className="px-4 py-2 bg-white text-black rounded-full text-[12px] font-black attractive-glow">🔥 {userData.yakarma} Yakarma</span><span className="px-3 py-2 bg-green-500/20 border border-green-500/30 text-green-400 rounded-full text-[11px] font-black">✓ REAL • #{totalUsers}</span></div></div></div><div className="mt-6 grid grid-cols-3 gap-2.5"><div className="bg-white/[0.06] border border-white/10 rounded-[16px] p-3 text-center"><p className="text-[20px] font-black text-white">{userData.totalPosts||0}</p><p className="text-[9px] font-bold tracking-[0.15em] text-white/50">YAKS</p></div><div className="bg-white/[0.06] border border-white/10 rounded-[16px] p-3 text-center"><p className="text-[20px] font-black text-white">{collegeCounts[userData.college]||0}</p><p className="text-[9px] font-bold tracking-[0.15em] text-white/50">REAL IN {userData.college}</p></div><div className="bg-white text-black rounded-[16px] p-3 text-center attractive-glow"><p className="text-[20px] font-black">{totalUsers}</p><p className="text-[9px] font-bold tracking-[0.15em] opacity-70">TOTAL REAL ✨</p></div></div><div className="mt-4 bg-white/5 border border-white/10 rounded-[14px] p-3"><p className="text-[10px] font-black tracking-widest text-white/40">REAL PER COLLEGE (LIVE)</p><div className="mt-2 grid grid-cols-2 gap-2">{COLLEGES.map(c=><div key={c.id} className={`rounded-[12px] p-2.5 border flex items-center justify-between ${c.id===userData.college?'bg-white text-black border-white attractive-glow':'bg-white/5 border-white/10 text-white/70'}`}><div className="flex items-center gap-2"><div className={`w-7 h-7 rounded-[8px] bg-gradient-to-br ${c.color} flex items-center justify-center text-[12px]`}>{c.emoji}</div><p className="font-black text-[11px]">{c.id}</p></div><p className="font-black text-[13px]">{collegeCounts[c.id]||0} real</p></div>)}</div></div></div></div>
