...
const COLLEGES = [
  {id:"SRET", label:"SRET", city:"Tirupati", color:"from-blue-600 to-cyan-500", emoji:"🎓", students:"2.1k"},
  {id:"SVCE", label:"SVCE", city:"Tirupati", color:"from-emerald-600 to-teal-500", emoji:"🚀", students:"1.8k"},
  {id:"BITS", label:"BITS", city:"Pilani", color:"from-violet-600 to-indigo-600", emoji:"⚡", students:"3.2k"},
  {id:"ST.JOHNS", label:"ST.JOHNS", city:"Tirupati", color:"from-orange-600 to-red-500", emoji:"🔥", students:"1.5k"},
  {id:"VEMU", label:"VEMU", city:"Chittoor", color:"from-amber-500 to-orange-500", emoji:"💎", students:"1.2k"},
  {id:"OTHER", label:"OTHER", city:"India", color:"from-zinc-700 to-zinc-900", emoji:"🌍", students:"5k+"},
];
const AVATARS = [
...
const Footer = () => (
  <div className="w-full py-8 flex flex-col items-center gap-2 border-t border-zinc-900/50 mt-8 bg-[#050507]">
    ...
    <p className="text-[10px] text-zinc-600">Attractive • Instant Feed • Real • Edit/Delete • Smooth</p>
  </div>
);
...
export default function YakApp(){
    ...
    return onAuthStateChanged(auth, async(u:any)=>{
      if(u){
        ...
        if(snap.empty){
          ...
          if(!raw.avatarBg ||!raw.avatarGlow){
            const f={avatar:raw.avatar||"👻", avatarBg:raw.avatarBg||"bg-zinc-900", avatarGlow:raw.avatarGlow||raw.avatarBg||"bg-zinc-900"};
            await updateDoc(doc(db,'users',raw.id),f); raw={...raw,...f};
          }
  ...
  // INSTANT COLLEGE FEED - Enter avvagane aa college posts ye
  useEffect(()=>{
  ...
  const handleDelete=async(y:any)=>{
    if(!confirm(`Delete yak from ${y.college}?`)) return;
    if(user?.uid!==y.uid) return;
    await deleteDoc(doc(db,'yaks',y.id)); await updateDoc(doc(db,'users',userData.id),{totalPosts:increment(-1), yakarma:increment(-5)}); setShowMenu(null);
  };
  const handleEdit=async()=>{
    if(!editingPost ||!editText.trim()) return;
    await updateDoc(doc(db,'yaks',editingPost.id),{text:String(editText.trim()), edited:true, editedAt:serverTimestamp()});
    ...
    <div className="min-h-screen bg-[#050507] text-white flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-[#050507]/80 backdrop-blur-xl border-b border-zinc-900/50">
        <div className="max-w-[600px] mx-auto px-5 h-[68px] flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center font-black text-black">Y</div><div><p className="font-black text-[15px] tracking-tighter leading-none">{userData.college} Herd • {yaks.length} yaks</p><p className="text-[11px] text-zinc-500 font-bold">Enter avvagane ne college posts • Attractive</p></div></div>
          ...
          return(
            <div key={y.id} className="bg-[#0a0a0f] border border-zinc-900 rounded-[26px] p-5 hover:border-zinc-800 transition-all relative">
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                {isOwn && <span className="ml-auto text-[10px] bg-white/10 px-2 py-1 rounded-full font-bold">Your Yak • ⋯ Edit/Delete</span>}
              </div>
      ...
      {editingPost && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xl z-[60] flex items-end justify-center p-3">
          <div className="bg-[#0a0a0f] border border-zinc-800 w-full max-w-[600px] rounded-t-[28px] p-6 pb-8">
            ...
            <div className="flex items-center justify-between"><h2 className="font-black text-[18px]">✏️ Edit Yak</h2><span className="text-[10px] bg-white/10 px-2 py-1 rounded-full">{editingPost.college}</span></div>
      ...
      {showProfile && (
        ...
        <div className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 rounded-[28px] p-6"><div className="flex gap-4"><div className={`w-20 h-20 rounded-[22px] ${userData.avatarBg} border-2 border-white/20 flex items-center justify-center text-3xl ${userData.avatarGlow}`}>{userData.avatar}</div><div><h2 className="font-black text-[22px] tracking-tighter leading-none">{userData.username} • Attractive ✨</h2><p className="text-[13px] font-bold text-white/70 mt-1">{userData.college} Herd • Yakarma {userData.yakarma}</p><div className="flex gap-2 mt-3"><span className="px-3 py-1.5 bg-white text-black rounded-full text-[11px] font-black">Yakarma {userData.yakarma} ✨</span><span className="px-3 py-1.5 bg-green-500/20 border border-green-500/30 text-green-400 rounded-full text-[10px] font-black">✓ REAL</span></div></div></div>
        <div className="mt-5 grid grid-cols-3 gap-2"><div className="bg-white/5 border border-white/10 rounded-[14px] p-3 text-center"><p className="text-[20px] font-black">{userData.totalPosts||0}</p><p className="text-[9px] font-bold tracking-widest text-zinc-500">YAKS</p></div><div className="bg-white/5 border border-white/10 rounded-[14px] p-3 text-center"><p className="text-[20px] font-black">{yaks.length}</p><p className="text-[9px] font-bold tracking-widest text-zinc-500">HERD</p></div><div className="bg-white/5 border border-white/10 rounded-[14px] p-3 text-center"><p className="text-[20px] font-black">{userData.yakarma}</p><p className="text-[9px] font-bold tracking-widest text-zinc-500">KARMA</p></div></div></div>
