...
const Footer = () => (<div className="w-full py-8 flex flex-col items-center gap-1 border-t border-white/[0.06] mt-8"><p className="text-[10px] tracking-[0.3em] font-bold text-white/40">SRET ONLY • A PRODUCTION BY ANESH • ONLY HEARTS</p><p className="text-[9px] text-white/20">Only Hearts Avatar - Text Only - No Vulgar - Delete Anytime</p></div>);
...
export default function YakFixed(){
  ...
  const createNotification=async(toUid:string, type:string, text:string, yakId?:string)=>{
    ...
    await addDoc(collection(db,'notifications'),{toUid, fromUid:user?.uid, fromUsername:userData?.anonymousName||"Anonymous", fromAvatar:userData?.avatar||"❤️", type, text, yakId:yakId||null, read:false, isAnonymous:true,isTextOnly:true,isClean:true,isHeartOnly:true, createdAt:serverTimestamp()});
  };
  ...
  const handlePollVote=async(y:any, idx:number)=>{ if(!userData) return; if(userData.pollVoted?.includes(y.id)){ showToast("Already voted as "+userData.anonymousName+" "+userData.avatar); return; } try{ const n=[...y.pollOptions]; n[idx].votes=(n[idx].votes||0)+1; await updateDoc(doc(db,'yaks',y.id),{pollOptions:n, totalVotes:increment(1)}); await updateDoc(doc(db,'users',userData.id),{pollVoted:arrayUnion(y.id)}); setUserData({...userData, pollVoted:[...(userData.pollVoted||[]), y.id]}); showToast("Voted as "+userData?.anonymousName+" "+userData?.avatar+" Clean"); }catch(e:any){ showToast(e.message); } };
  const handlePost=async()=>{
    ...
    try{
      const anonName = userData.anonymousName; const anonAvatar = userData.avatar||selectedAvatar||"❤️";
      const payload:any={ text:txt, uid:user.uid, anonymousName:anonName, username:anonName, avatar:anonAvatar, college:"SRET", type:yakType, isAnonymous:true,isTextOnly:true,isClean:true,isHeartOnly:true, likes:0, dislikes:0, commentsCount:0, reports:0, hidden:false, createdAt:serverTimestamp() };
      ...
      const hashtagsInText=txt.match(/#\w+/g); if(hashtagsInText) payload.hashtags=hashtagsInText.map((h:string)=>h.toLowerCase());
      ...
      setNewYak(''); setPollOptions(['','']); setMarketPrice(''); setPyqSubject(''); setYakType('yak'); setScreen('feed'); showToast(`Posted as ${anonName} ${anonAvatar} Text Only Clean`);
    }catch(e:any){ showToast(e.message); }finally{ setPosting(false); }
  };
  const handleDelete=async(y:any)=>{ const isOwn=user?.uid===y.uid; if(!isOwn){ showToast("Only your post - "+userData?.anonymousName+" "+userData?.avatar); return; } if(!confirm(`DELETE as ${userData?.anonymousName} ${userData?.avatar}?\n"${y.text.slice(0,60)}"\nText Only Clean`)) return; try{ await deleteDoc(doc(db,'yaks',y.id)); await updateDoc(doc(db,'users',userData.id),{totalPosts:increment(-1)}); showToast(`Deleted as ${userData?.anonymousName} ${userData?.avatar} Clean`); }catch(e:any){ showToast(e.message); } setShowMenu(null); };
  const handleEdit=async()=>{ if(!editingPost) return; if(!editText.trim()) return; if(containsVulgar(editText)){ showToast("No vulgar "+userData?.avatar); return; } try{ await updateDoc(doc(db,'yaks',editingPost.id),{text:editText.trim(), edited:true}); showToast("Edited as "+userData?.anonymousName+" "+userData?.avatar); }catch(e:any){ showToast(e.message); } setEditingPost(null); setEditText(''); setShowMenu(null); };
  const handleReport=async(y:any, reasonArg?:string)=>{ const finalReason=reasonArg||reportReason; if(!userData) return; if(userData.reportedPosts?.includes(y.id)){ showToast("Already reported as "+userData?.anonymousName+" "+userData?.avatar); setReportingPost(null); setShowMenu(null); return; } if(!finalReason){ showToast("Select reason - "+userData?.anonymousName+" "+userData?.avatar); return; } try{ await addDoc(collection(db,'reports'),{yakId:y.id, yakText:y.text.slice(0,200), yakUid:y.uid, yakAnonymousName:y.anonymousName, reportedBy:user.uid, reporterAnonymousName:userData?.anonymousName, reporterAvatar:userData?.avatar, reason:finalReason, status:"pending", isTextOnly:true,isClean:true,isHeartOnly:true, createdAt:serverTimestamp()}); await updateDoc(doc(db,'yaks',y.id),{reports:increment(1)}); await updateDoc(doc(db,'users',userData.id),{reportedPosts:arrayUnion(y.id)}); setUserData({...userData, reportedPosts:[...(userData.reportedPosts||[]), y.id]}); showToast(`Reported as ${userData?.anonymousName} ${userData?.avatar} Clean`); setReportingPost(null); setReportReason(''); setShowMenu(null); }catch(e:any){ showToast(e.message); } };
  ...
  const handleCommentPost = async (yId:string) => {
    if(!commentText.trim() ||!user ||!userData) return; const text = commentText.trim(); if(containsVulgar(text)){ showToast("No vulgar As "+userData?.anonymousName+" "+userData?.avatar); return; } const payload:any = { text, uid: user.uid, username: userData?.anonymousName, anonymousName: userData?.anonymousName, avatar: userData?.avatar||"❤️", parentId: replyTo? replyTo.id : null, replyToUsername: replyTo? replyTo.anonymousName : null, isAnonymous:true,isTextOnly:true,isClean:true,isHeartOnly:true, createdAt: serverTimestamp() }; setCommentText(''); const temp = replyTo; setReplyTo(null);
  ...
  const handleStartDm = async (otherUid:string, yakId?:string)=>{
    ...
    try{
      const newChat=await addDoc(collection(db,'dms'),{
        ...
        lastMessage:`Started private chat as ${userData?.anonymousName} ${userData?.avatar} - Text Only Clean`,
      ...
      setFeedTab('dm');
      showToast(`Private DM as ${userData?.anonymousName} ${userData?.avatar}`);
    }catch(e:any){ showToast(e.message); }
  };
  ...
  const handleBlockUser=async(targetUid:string)=>{ if(!userData || targetUid===user?.uid) return; if(!confirm(`Block ${userData?.anonymousName} ${userData?.avatar}? Only Hearts`)) return; try{ await updateDoc(doc(db,'users',userData.id),{blockedUsers:arrayUnion(targetUid)}); setBlockedUsers([...blockedUsers, targetUid]); setShowMenu(null); showToast("Blocked as "+userData?.anonymousName+" "+userData?.avatar); }catch(e:any){ showToast(e.message); } };
  const handleUnblockUser=async(targetUid:string)=>{ try{ await updateDoc(doc(db,'users',userData.id),{blockedUsers:arrayRemove(targetUid)}); setBlockedUsers(blockedUsers.filter(id=>id!==targetUid)); showToast("Unblocked "+userData?.avatar); }catch(e:any){ showToast(e.message); } };
  const handleCrushSubmit=async()=>{ const roll=crushRoll.trim().toUpperCase(); if(!roll){ showToast("Enter roll "+userData?.anonymousName+" "+userData?.avatar); return; } if(containsVulgar(roll)){ showToast("No vulgar "+userData?.avatar); return; } try{ await addDoc(collection(db,'crushes'),{fromUid:user.uid, fromAnonymousName:userData?.anonymousName, fromAvatar:userData?.avatar, toRoll:roll, matched:false, isHeartOnly:true, createdAt:serverTimestamp()}); setCrushRoll(''); showToast("Crush saved as "+userData?.anonymousName+" "+userData?.avatar); }catch(e:any){ showToast(e.message); } };
  const handleCancelCrush=async(crushId:string)=>{ if(!confirm(`Cancel crush as ${userData?.anonymousName} ${userData?.avatar}?`)) return; try{ await deleteDoc(doc(db,'crushes',crushId)); showToast("Cancelled "+userData?.avatar); }catch(e:any){ showToast(e.message); } };
    ...
    return (
      <div key={c.id} className={`${isReply? 'ml-6 border-l-2 border-white/15 pl-3' : ''} mt-3`}>
        <div className="flex gap-2.5">
          <div className={`bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white shrink-0 ${isReply? 'w-6 h-6 text-[10px]' : 'w-7 h-7 text-xs'}`}>{c.avatar||"❤️"}</div>
          <div className="flex-1">
            <div className={`bg-white/[0.05] border rounded-[14px] px-4 py-2.5 ${isOwnComment?'border-white/20 bg-white/[0.08]':'border-white/10'}`}>
              <p className="text-[10px] font-bold text-white/40">{anonDisplay} {isOwnComment?`YOU (${userData?.anonymousName} ${userData?.avatar}) Can DELETE`:''}</p>
            ...
            <div className="flex gap-3 mt-1.5 ml-1 items-center">
              <button onClick={()=>setReplyTo(c)} className="text-[11px] font-bold text-white/30">Reply as {userData?.anonymousName} {userData?.avatar} Clean</button>
              <button onClick={()=>handleStartDm(c.uid)} className="text-[11px] font-bold text-green-400">DM {anonDisplay.split(' ')[0]} {userData?.avatar} Clean</button>
              {isOwnComment && <button onClick={async()=>{ if(confirm(`Delete comment as ${userData?.anonymousName} ${userData?.avatar}? Clean`)){ await deleteDoc(doc(db,'yaks/'+activePost+'/comments/'+c.id)); await updateDoc(doc(db,'yaks',activePost as string), {commentsCount: increment(-1)}); } }} className="text-[11px] font-bold text-red-400">Delete {userData?.avatar} Clean</button>}
            </div>
    ...
    <div className="min-h-screen bg-[#0a0a0b] text-white flex flex-col">
      ...
      <div className="sticky top-0 z-20 bg-[#0a0a0b]/80 backdrop-blur-2xl border-b border-white/10">
        <div className="max-w-[600px] mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-[13px] leading-none text-white">{userData?.anonymousName||"Anonymous"} {userData?.avatar||"❤️"} • {yaks.length} • TEXT ONLY CLEAN</p>
              <p className="text-[10px] text-white/40">{userData?.anonymousName} {userData?.avatar} No real name Text only Clean No vulgar Delete anytime • {totalUsers} verified • Only Hearts</p>
            </div>
          </div>
        ...
        <div className="max-w-[600px] mx-auto px-3 pb-2 flex gap-2">
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={`Search as ${userData?.anonymousName||"Anonymous"} ${userData?.avatar||"❤️"} #hashtag Text only Clean`} className="flex-1 h-9 bg-white/5 border border-white/10 rounded-full px-4 text-xs outline-none text-white placeholder:text-white/30 focus:border-white" />
        </div>
      ...
      <div className="max-w-[600px] mx-auto w-full flex-1 p-3 pb-[84px] space-y-3">
        {hashtags.length>0 &&!['dm','top','crush','market','pyq'].includes(feedTab) && (
          <div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">TRENDING HASHTAGS {userData?.avatar||"❤️"} ONLY HEARTS TEXT ONLY CLEAN {userData?.anonymousName}</p><div className="flex gap-2 mt-3 flex-wrap">{hashtags.map((h:any)=><button key={h.tag} onClick={()=>setSearchQuery(h.tag)} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-[11px] font-bold text-white/60 hover:bg-white hover:text-black">{h.tag} {h.count} {userData?.avatar||"❤️"} Clean</button>)}</div></div>
        )}
        {feedTab==='crush' && (
          <div className="space-y-3">
            <div className="bg-gradient-to-br from-pink-500/10 to-purple-500/10 border-2 border-pink-500/20 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-pink-400">CRUSH MATCHER 💘 {userData?.avatar||"❤️"} ONLY HEARTS TEXT ONLY CLEAN {userData?.anonymousName}</p><div className="flex gap-2 mt-4"><input value={crushRoll} onChange={e=>setCrushRoll(e.target.value.toUpperCase())} placeholder={`Crush Roll as ${userData?.anonymousName} ${userData?.avatar||"❤️"} 21CS masked Clean`} className="flex-1 bg-black/30 border-2 border-pink-500/20 rounded-full px-5 h-11 text-[13px] outline-none text-white"/><button onClick={handleCrushSubmit} className="px-6 h-11 bg-pink-500 text-white rounded-full font-bold text-xs">Add as {userData?.anonymousName?.split(' ')[0]} {userData?.avatar||"❤️"} Clean</button></div></div>
            <Footer/>
          </div>
        )}
        {feedTab==='market' && (
          <div className="space-y-3">
            ...
            {displayMarketYaks.map((y:any)=>{
              ...
              return(<div key={y.id} className={`bg-white/[0.04] border-2 rounded-[20px] p-5 ${isOwn?'border-white/20':'border-white/10'}`}><div className="flex justify-between"><p className="font-bold text-[13px] text-white">{authorName} {y.avatar||"❤️"} {isOwn?`YOU (${userData?.anonymousName} ${userData?.avatar}) Can DELETE`:''} 🛒 TEXT ONLY CLEAN</p><span className="px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-full text-[11px] font-bold text-green-400">₹{y.price} {y.avatar||"❤️"} Clean</span></div><p className="text-[14px] text-white mt-3 whitespace-pre-wrap break-words">{y.text}</p><div className="flex gap-2 mt-4"><button onClick={()=>handleStartDm(y.uid, y.id)} className="flex-1 h-10 bg-white text-black rounded-full font-bold text-xs">DM {authorName.split(' ')[0]} as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button>{isOwn && <button onClick={()=>handleDelete(y)} className="px-4 h-10 bg-red-500 text-white rounded-full font-bold text-xs">DELETE {userData?.avatar||"❤️"} Clean</button>}</div></div>);
            })}
            <Footer/>
          </div>
        )}
        {feedTab==='pyq' && (
          <div className="space-y-3">
            ...
            {displayPyqYaks.map((y:any)=>{ const isOwn=y.uid===user?.uid; return(<div key={y.id} className={`bg-white/[0.04] border-2 rounded-[20px] p-5 ${isOwn?'border-white/20':'border-white/10'}`}><p className="font-bold text-[13px] text-white">{y.anonymousName||"Anonymous"} {y.avatar||"❤️"} {isOwn?`YOU (${userData?.anonymousName}) ${userData?.avatar} Clean`:''} Text only Clean</p><p className="text-[14px] text-white mt-3 whitespace-pre-wrap break-words">{y.text}</p><div className="flex gap-2 mt-4"><button onClick={()=>handleStartDm(y.uid)} className="px-4 h-8 bg-white text-black rounded-full text-xs font-bold">DM as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button>{isOwn && <button onClick={()=>handleDelete(y)} className="px-4 h-8 bg-red-500 text-white rounded-full text-xs font-bold">DELETE {userData?.avatar||"❤️"} Clean</button>}</div></div>); })}
            <Footer/>
          </div>
        )}
        ...
        {feedTab==='dm'? (
          <div className="space-y-3">
            {!activeDm? (
              <><div className="bg-white/[0.05] border-2 border-white/10 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">1to1 PRIVATE DM - SRET ONLY {userData?.avatar||"❤️"} ONLY HEARTS TEXT ONLY CLEAN</p><p className="font-black mt-1 text-white">Private Chats - {dmChats.length} • Only You 2 • {userData?.anonymousName} {userData?.avatar||"❤️"} • Text Only Clean</p></div>
              {dmChats.map((chat:any)=><button key={chat.id} onClick={()=>setActiveDm(chat)} className="w-full bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center text-left"><div className="flex gap-3 items-center"><div className="w-10 h-10 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">{userData?.avatar||"❤️"}</div><div><p className="font-bold text-[13px] text-white">Private Chat {userData?.avatar||"❤️"} Only You 2 • Text Only Clean</p><p className="text-[11px] text-white/40 truncate max-w-[200px]">{chat.lastMessage} • {userData?.avatar||"❤️"} Clean</p></div></div><span className="px-2 py-1 bg-green-500/20 rounded-full text-[8px] font-bold text-green-400">1to1 PRIVATE {userData?.avatar||"❤️"}</span></button>)}
              {dmChats.length===0 && <div className="py-24 text-center bg-white/[0.03] border-2 border-white/10 rounded-[24px]"><p className="font-black text-[18px] text-white">No Private DMs yet {userData?.avatar||"❤️"} Only Hearts</p><p className="text-[11px] text-white/30 mt-1">Start private DM from any post as {userData?.anonymousName} {userData?.avatar||"❤️"} - Text Only Clean No Vulgar</p></div>}<Footer/></>
            ) : (
              <div className="bg-white/[0.03] border-2 border-white/10 rounded-[24px] flex flex-col h-[70vh]">
                <div className="p-4 border-b-2 border-white/10 flex justify-between items-center"><div className="flex gap-3 items-center"><button onClick={()=>setActiveDm(null)} className="w-8 h-8 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">←</button><div><p className="font-bold text-[13px] text-white">1to1 Private Chat {userData?.avatar||"❤️"} {userData?.anonymousName} Text Only Clean</p><p className="text-[9px] text-green-400">Only you 2 can see - Only Hearts {userData?.avatar||"❤️"} Clean</p></div></div><span className="px-2 py-1 bg-green-500/20 border border-green-500/30 rounded-full text-[8px] font-bold text-green-400">PRIVATE {userData?.avatar||"❤️"}</span></div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3">{dmMessages.map((m:any)=><div key={m.id} className={`flex ${m.uid===user?.uid?'justify-end':'justify-start'}`}><div className={`max-w-[70%] rounded-[16px] px-4 py-2.5 ${m.uid===user?.uid?'bg-white text-black':'bg-white/10 border border-white/10 text-white'}`}><p className="text-[10px] font-bold opacity-60">{m.uid===user?.uid? userData?.anonymousName+" "+(userData?.avatar||"❤️") : "Anonymous "+(m.avatar||"❤️")} Clean</p><p className="text-[13px] leading-[1.4]">{m.text}</p><p className="text-[8px] opacity-50 mt-1">{m.uid===user?.uid?`You (${userData?.anonymousName} ${userData?.avatar||"❤️"})`:'Anonymous '+ (m.avatar||"❤️")} • Private Clean</p></div></div>)}</div>
                <div className="p-3 border-t-2 border-white/10 flex gap-2"><input value={dmText} onChange={e=>setDmText(e.target.value)} placeholder={`Private message as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean No vulgar`} className="flex-1 bg-white/5 border-2 border-white/10 rounded-full px-5 h-11 text-[13px] outline-none text-white placeholder:text-white/30 focus:border-white" onKeyDown={e=>{ if(e.key==='Enter') handleSendDm(); }}/><button onClick={handleSendDm} disabled={!dmText.trim()} className={`w-11 h-11 rounded-full font-bold ${!dmText.trim()?'bg-white/5 text-white/20':'bg-white text-black'}`}>Go {userData?.avatar||"❤️"}</button></div>
              </div>
            )}
          </div>
        ) : feedTab==='top'? (
          <div className="space-y-3">
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">SRET TOP ANONYMOUS {userData?.avatar||"❤️"} ONLY HEARTS TEXT ONLY CLEAN • Private DM</p><p className="font-black mt-1 text-white">Top Anonymous {userData?.avatar||"❤️"} - Only Hearts - Text Only Clean</p></div>
            {leaderboard.map((u:any,i:number)=><div key={u.id} className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><span className="w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-white">{i+1}</span><span className="w-9 h-9 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">{u.avatar||"❤️"}</span><div><p className="font-bold text-[13px] text-white">{u.anonymousName||`Anonymous Student ${i+1}`} {u.avatar||"❤️"} {i===0?'👑':''} {u.uid===user?.uid?`YOU (${userData?.anonymousName} ${userData?.avatar||"❤️"}) Clean`:''} • Only Hearts Clean</p><p className="text-[10px] text-white/40">{u.totalPosts||0} posts • SRET Verified {u.avatar||"❤️"} Text Only Clean No Vulgar</p></div></div><p className="font-black text-sm text-white">{u.yakarma} {u.avatar||"❤️"}</p></div>)}<Footer/></div>
        ) :!['crush','market','pyq','dm','top'].includes(feedTab) && (
          <>
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-10 h-10 bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white">{userData?.avatar||"❤️"}</div><div><p className="font-bold text-[13px] text-white">Anonymous {userData?.avatar||"❤️"} Only Hearts - 1to1 Private Text Only Clean</p><p className="text-[11px] text-white/40">No One Sees Other Chat • SRET ONLY • {userData?.anonymousName} {userData?.avatar||"❤️"} Text Only Clean No Vulgar</p></div></div><span className="px-3 py-1.5 rounded-full bg-green-500/20 border border-green-500/30 text-[10px] font-bold text-green-400">PRIVATE DM {userData?.avatar||"❤️"}</span></div>
                ...
                <div key={y.id} className={`bg-white/[0.04] border-2 rounded-[20px] p-5 ${isOwn?'border-white/20 bg-white/[0.06]':'border-white/10'}`}>
                  <div className="flex justify-between items-start">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <div className="flex gap-2 items-center flex-wrap">
                          <p className="font-bold text-[13px] text-white">{authorName} {y.avatar||"❤️"} {isOwn?`YOU (${userData?.anonymousName} ${userData?.avatar||"❤️"}) Can DELETE`:''}</p>
                          ...
                          <span className="px-1.5 py-0.5 rounded-full text-[7px] font-bold bg-green-500/20 text-green-400">ANON {y.avatar||"❤️"} ONLY HEARTS CLEAN</span>
                          {isOwn && <span className="px-1.5 py-0.5 rounded-full text-[7px] font-bold bg-blue-500 text-white">DELETE {y.avatar||"❤️"} CLEAN</span>}
                        </div>
                        <p className="text-[10px] text-white/30 mt-0.5">Anonymous {y.avatar||"❤️"} Only Hearts - {score} • Text only Clean No vulgar • {y.hashtags?.join(' ')||''} • {y.avatar||"❤️"}</p>
                      </div>
                    </div>
                    <div className="relative flex gap-2">
                      ...
                      {showMenu===y.id && <div className="absolute right-0 top-10 w-[240px] bg-black border-2 border-white/10 rounded-2xl p-2 z-20 shadow-2xl">{isOwn? (<><button onClick={()=>{ setEditingPost(y); setEditText(y.text); setShowMenu(null); }} className="w-full text-left px-4 py-3 rounded-xl text-xs font-bold bg-white/5 text-white">Edit as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button><button onClick={()=>handleDelete(y)} className="w-full text-left px-4 py-3 rounded-xl text-xs font-bold bg-red-500 text-white mt-2">DELETE MY POST {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button></>) : (<><button onClick={()=>handleStartDm(y.uid, y.id)} className="w-full text-left px-4 py-3 rounded-xl text-xs font-bold bg-green-500/10 border border-green-500/20 text-green-400">Private DM {y.avatar||"❤️"} Only You 2 as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button><button onClick={()=>{ setReportingPost(y); setShowMenu(null); }} className="w-full text-left px-4 py-3 rounded-xl text-xs font-bold bg-white/5 border border-white/10 text-white/60 mt-2">Report {y.reports||0}/5 → Review Clean {y.avatar||"❤️"}</button><button onClick={()=>handleBlockUser(y.uid)} className="w-full text-left px-4 py-3 rounded-xl text-xs font-bold bg-red-500/10 border border-red-500/20 text-red-400 mt-2">Block {authorName} {y.avatar||"❤️"} Clean</button></>)}<button onClick={()=>setShowMenu(null)} className="w-full mt-2 py-2 rounded-xl text-[11px] text-white/30">Cancel {userData?.avatar||"❤️"}</button></div>}
                    </div>
                  </div>
                  <p className="text-[15px] mt-4 leading-[1.5] text-white whitespace-pre-wrap break-words">{y.text}</p>
                  {y.hidden && <div className="mt-3 bg-yellow-500/10 border-2 border-yellow-500/20 rounded-xl p-3"><span className="text-yellow-400 text-[10px] font-bold">HIDDEN - {y.reports}/5 - Only you see {y.avatar||"❤️"} Clean</span></div>}
                  {isPoll && y.pollOptions && (<div className="mt-4 bg-white/[0.03] border-2 border-white/10 rounded-[16px] p-4"><div className="flex justify-between items-center mb-3"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">POLL {y.avatar||"❤️"} ONLY HEARTS - PRIVATE VOTE - {userData?.anonymousName} {userData?.avatar||"❤️"} CLEAN</p><span className="px-2 py-1 bg-white text-black rounded-full text-[9px] font-bold">{y.totalVotes||0} VOTES • {y.avatar||"❤️"} Clean</span></div><div className="space-y-2.5">{[...y.pollOptions].sort((a:any,b:any)=> (b.votes||0)-(a.votes||0)).map((opt:any,idx:number)=>{ const total=y.totalVotes||1; const percent=Math.round((opt.votes/total)*100)||0; const rank = idx+1; return (<button key={idx} onClick={()=>handlePollVote(y,y.pollOptions.indexOf(opt))} disabled={!!hasVoted} className="w-full relative overflow-hidden rounded-xl border-2 text-left p-0 border-white/10"><div className="absolute left-0 top-0 bottom-0 bg-white/10" style={{width:`${hasVoted? percent: 0}%`}}></div><div className="relative flex justify-between items-center p-3"><div className="flex gap-2 items-center"><span className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold bg-white/10 text-white">#{rank}</span><span className="text-[13px] font-bold">{opt.text} {y.avatar||"❤️"} Clean</span></div><div className="text-right"><p className="text-[12px] font-black">{hasVoted? `${percent}%` : `${opt.votes||0} votes`} {y.avatar||"❤️"}</p></div></div></button> )})}</div></div>)}
                  <div className="flex gap-2.5 mt-5 items-center flex-wrap">
                    ...
                    <button onClick={()=>{ setActivePost(activePost===y.id?null:y.id); setReplyTo(null); }} className="px-4 h-9 rounded-full text-xs bg-white/5 border border-white/10 text-white/40">Comments {y.commentsCount||0} {y.avatar||"❤️"} Clean</button>
                    <button onClick={()=>handleStartDm(y.uid, y.id)} className="px-4 h-9 rounded-full text-xs bg-green-500/10 border border-green-500/20 text-green-400 font-bold">DM {authorName.split(' ')[0]} {y.avatar||"❤️"} as {userData?.anonymousName?.split(' ')[0]} {userData?.avatar||"❤️"} Clean</button>
                    {isOwn && <button onClick={()=>handleDelete(y)} className="px-4 h-9 rounded-full text-xs bg-red-500 text-white font-bold">DELETE {y.avatar||"❤️"} Clean</button>}
                  </div>
                  {activePost===y.id && (<div className="mt-5 border-t-2 border-white/10 pt-4 space-y-1"><p className="text-[10px] font-bold tracking-widest text-white/30 mb-3">REPLIES {comments.length} {userData?.avatar||"❤️"} Only Hearts Clean No vulgar</p>{replyTo && (<div className="bg-white/10 border-2 border-white/20 rounded-xl px-4 py-2.5 flex justify-between items-center mb-3"><p className="text-[11px] text-white">Replying as {userData?.anonymousName} {userData?.avatar||"❤️"} to {replyTo.anonymousName} {replyTo.avatar||"❤️"}: {replyTo.text.slice(0,30)} Clean</p><button onClick={()=>setReplyTo(null)} className="w-6 h-6 bg-white/10 rounded-full flex items-center justify-center text-white text-xs">X</button></div>)}<div className="max-h-[420px] overflow-y-auto pr-1">{nestedTree.length===0 && <p className="text-xs text-white/20 text-center py-8">No comments yet as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</p>}{nestedTree.map((c:any)=>renderComment(c,0))}</div><div className="flex gap-2.5 mt-4"><input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder={`Comment as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean No vulgar`} className="flex-1 bg-white/5 border-2 border-white/10 rounded-full px-5 h-11 text-[13px] outline-none text-white placeholder:text-white/30 focus:border-white" onKeyDown={e=>{ if(e.key==='Enter'){ handleCommentPost(y.id); } }}/><button onClick={()=>handleCommentPost(y.id)} disabled={!commentText.trim()} className={`w-11 h-11 rounded-full font-bold flex items-center justify-center ${!commentText.trim()?'bg-white/5 text-white/20 border border-white/5':'bg-white text-black'}`}>Go {userData?.avatar||"❤️"}</button></div></div>)}
                </div>
              );
            })}
            {(feedTab==='new'? filteredYaks : feedTab==='meme'? displayMemeYaks : displayHotYaks).length===0 && <div className="py-24 text-center bg-white/[0.03] border-2 border-white/10 rounded-[24px]"><div className="w-24 h-24 bg-white/5 border-2 border-white/10 rounded-[24px] mx-auto flex items-center justify-center text-4xl">{userData?.avatar||"❤️"}</div><p className="font-black mt-6 text-[18px] text-white">No posts yet {userData?.avatar||"❤️"} - Only Hearts</p><p className="text-[11px] text-white/30 mt-1">Post as {userData?.anonymousName} {userData?.avatar||"❤️"} #hashtag Text only Clean No vulgar</p><button onClick={()=>setScreen('create')} className="mt-6 bg-white text-black px-8 h-11 rounded-full text-[13px] font-bold">Create as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button></div>}
            <Footer/>
          </>
        )}
      </div>
      <div className="fixed bottom-0 left-0 right-0 bg-[#0a0a0b]/90 backdrop-blur-2xl border-t-2 border-white/10"><div className="max-w-[600px] mx-auto px-6 h-[72px] flex items-center justify-between"><button onClick={()=>{ setFeedTab('new'); setActiveDm(null); }} className="flex flex-col items-center gap-1.5"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-bold ${feedTab==='new'?'bg-white text-black':'bg-white/5 text-white/30 border border-white/10'}`}>{userData?.avatar||"❤️"}</div><span className="text-[8px] font-bold tracking-widest text-white/30">SRET {yaks.length} {userData?.avatar||"❤️"} CLEAN DELETE</span></button><button onClick={()=>setScreen('create')} className="w-[56px] h-[56px] bg-white text-black rounded-full flex items-center justify-center text-[24px] font-black">{userData?.avatar||"❤️"}</button><button onClick={()=>setShowProfile(true)} className="flex flex-col items-center gap-1.5"><div className="w-7 h-7 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-xs text-white relative">{userData?.avatar||"❤️"}{unreadCount>0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full"></span>}</div><span className="text-[8px] font-bold tracking-widest text-white/30">SRET {userData?.yakarma||0} {userData?.avatar||"❤️"} CLEAN</span></button></div></div>
          ...
          <div className="max-w-[600px] mx-auto w-full flex flex-col h-full bg-[#0a0a0b]">
            <div className="p-5 flex items-center justify-between border-b-2 border-white/10">
              ...
              <div className="text-center">
                <p className="text-[11px] font-bold tracking-widest text-white">SRET ONLY {userData?.avatar||"❤️"} ONLY HEARTS TEXT ONLY CLEAN</p>
                <p className="text-[10px] text-white/30">{userData?.anonymousName} {userData?.avatar||"❤️"} Text only 300 chars • No vulgar • Delete anytime • {userData?.avatar||"❤️"}</p>
              </div>
              <button onClick={handlePost} disabled={posting||!newYak.trim()} className={`px-6 h-10 rounded-full font-bold text-[13px] ${posting||!newYak.trim()?'bg-white/5 text-white/20 border-2 border-white/5':'bg-white text-black'}`}>{posting?`Posting as ${userData?.anonymousName?.split(' ')[0]} ${userData?.avatar||"❤️"}...`:`Post as ${userData?.anonymousName?.split(' ')[0]} ${userData?.avatar||"❤️"} Clean`}</button>
            </div>
            <div className="p-3 flex gap-2 border-b-2 border-white/5 overflow-x-auto bg-white/[0.02]">
              <button onClick={()=>setYakType('yak')} className={`px-5 h-9 rounded-full text-xs font-bold border-2 whitespace-nowrap ${yakType==='yak'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>Talk {userData?.avatar||"❤️"} Clean</button>
            ...
            <div className="p-6 flex-1 overflow-y-auto bg-[#0a0a0b]">
              <div className="flex gap-3 mb-6">
                ...
                <div>
                  <p className="font-bold text-[14px] text-white">{userData?.anonymousName} {userData?.avatar||"❤️"} Anonymous - Only Hearts - Text Only Clean No Vulgar</p>
                  <p className="text-[11px] text-white/40">Posting as {userData?.anonymousName} {userData?.avatar||"❤️"} - No real name - Text only 300 chars - No pic - No vulgar - Clean - Delete anytime</p>
                </div>
              </div>
              {yakType==='market' && <div className="flex gap-2 mb-4"><input value={marketPrice} onChange={e=>setMarketPrice(e.target.value)} placeholder={`Price as ${userData?.anonymousName} ${userData?.avatar||"❤️"} e.g. 250 Clean`} className="w-[160px] p-4 bg-white/[0.03] border-2 border-green-500/20 rounded-xl text-sm outline-none text-white"/><p className="text-[10px] text-green-400 flex items-center">Price {userData?.avatar||"❤️"} Clean</p></div>}
              {yakType==='pyq' && <div className="flex gap-2 mb-4"><input value={pyqSubject} onChange={e=>setPyqSubject(e.target.value.toUpperCase())} placeholder={`Subject as ${userData?.anonymousName} ${userData?.avatar||"❤️"} M1 DBMS Clean`} className="flex-1 p-4 bg-white/[0.03] border-2 border-blue-500/20 rounded-xl text-sm outline-none text-white uppercase"/><p className="text-[10px] text-blue-400 flex items-center">{userData?.avatar||"❤️"} Clean</p></div>}
              <textarea value={newYak} onChange={e=>setNewYak(e.target.value)} placeholder={`${yakType==='market'? `What selling as ${userData?.anonymousName} ${userData?.avatar||"❤️"}? e.g. Drafter - Good condition - Text only Clean No vulgar #marketplace ${userData?.avatar||"❤️"} Clean` : yakType==='pyq'? `Describe PYQ as ${userData?.anonymousName} ${userData?.avatar||"❤️"}: e.g. DBMS 2023 Mid-1 - Important Qs - Text only Clean No vulgar #pyq #dbms ${userData?.avatar||"❤️"} Clean` : `Talk as ${userData?.anonymousName} ${userData?.avatar||"❤️"} about SRET... Use #hashtag like #SRET #Exams - Text only 300 chars - No pic - No vulgar - Clean - Delete anytime ${userData?.avatar||"❤️"} Only Hearts`}`} autoFocus className="w-full bg-transparent text-[19px] leading-[1.45] outline-none placeholder:text-white/20 resize-none min-h-[140px] text-white" maxLength={300}/>
              <p className="text-[10px] text-white/30 mt-2">{newYak.length}/300 • As {userData?.anonymousName} {userData?.avatar||"❤️"} • Text only Clean No vulgar • Delete anytime • Only Hearts</p>
              {yakType==='poll' && (<div className="mt-6 space-y-3"><p className="text-[10px] text-white/30 font-bold">POLL OPTIONS {userData?.avatar||"❤️"} ONLY HEARTS - Text only Clean No vulgar - {userData?.anonymousName} {userData?.avatar||"❤️"}</p>{pollOptions.map((opt,idx)=><div key={idx} className="flex gap-2"><input value={opt} onChange={e=>{ const n=[...pollOptions]; n[idx]=e.target.value; setPollOptions(n); }} placeholder={`Option ${idx+1} as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean No vulgar`} className="flex-1 p-4 bg-white/[0.03] border-2 border-white/10 rounded-xl text-sm outline-none focus:border-white text-white"/><button onClick={()=>setPollOptions(pollOptions.filter((_,i)=>i!==idx))} className="w-12 h-12 bg-white/5 border-2 border-white/10 rounded-xl flex items-center justify-center text-white/40">X</button></div>)}{pollOptions.length<4 && <button onClick={()=>setPollOptions([...pollOptions,''])} className="w-full p-3 bg-white/[0.03] border-2 border-dashed border-white/10 rounded-xl text-xs font-bold text-white/40">Add Option as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button>}</div>)}
              <div className="mt-6 bg-white/[0.03] border-2 border-white/10 rounded-[16px] p-4">
                <p className="text-[11px] font-bold text-white/40">TEXT ONLY - NO IMAGE - ONLY HEARTS AVATAR {userData?.avatar||"❤️"} - {userData?.anonymousName} Clean</p>
                <p className="text-[10px] text-white/30 mt-1">Only Hearts Avatar - Text only 300 chars - No pic - No vulgar - Clean - Delete anytime - {userData?.avatar||"❤️"} Only Hearts</p>
              </div>
            </div>
            <div className="p-5 border-t-2 border-white/5 bg-white/[0.02]"><div className="bg-green-500/5 border-2 border-green-500/10 rounded-xl p-4 flex gap-3 items-center"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><p className="text-[11px] text-white/50"><span className="font-bold text-green-400">{userData?.anonymousName} {userData?.avatar||"❤️"} 1to1 PRIVATE:</span> No one can see other chat - Only you 2 - SRET ONLY {userData?.avatar||"❤️"} Only Hearts - Text Only Clean No Vulgar Delete Anytime</p></div><Footer/></div>
          </div>
        </div>
      )}

      {editingPost && <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center p-4"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 pb-8 shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><h3 className="font-black text-[16px] text-white">Edit as {userData?.anonymousName} {userData?.avatar||"❤️"} - Text Only Clean No Vulgar - Only Hearts</h3><textarea value={editText} onChange={e=>setEditText(e.target.value)} className="w-full mt-5 bg-white/[0.05] border-2 border-white/10 rounded-xl p-4 text-[15px] outline-none min-h-[120px] resize-none text-white focus:border-white" maxLength={300}/><p className="text-[10px] text-white/30 mt-2">{editText.length}/300 {userData?.avatar||"❤️"} Clean</p><div className="flex gap-3 mt-6"><button onClick={()=>{ setEditingPost(null); setEditText(''); }} className="flex-1 h-12 bg-white/5 border-2 border-white/10 rounded-full font-bold text-xs text-white">Cancel {userData?.avatar||"❤️"}</button><button onClick={handleEdit} disabled={!editText.trim()||containsVulgar(editText)} className={`flex-1 h-12 rounded-full font-bold text-xs ${!editText.trim()||containsVulgar(editText)?'bg-white/5 text-white/20 border-2 border-white/5':'bg-white text-black'}`}>Save as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button></div></div></div>}

      {showNotifications && <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><div className="flex justify-between items-center"><h3 className="font-black text-[16px] text-white">Notifications {userData?.avatar||"❤️"} {unreadCount>0? `(${unreadCount} new)` : ''} • {userData?.anonymousName} {userData?.avatar||"❤️"} Only Hearts Clean</h3><button onClick={()=>setShowNotifications(false)} className="w-8 h-8 bg-white/5 border border-white/10 rounded-full flex items-center justify-center text-white">X</button></div><div className="mt-6 space-y-3">{notifications.map((n:any)=><div key={n.id} className={`p-4 rounded-[16px] border-2 ${!n.read?'bg-white/10 border-white/20':'bg-white/[0.03] border-white/10'}`}><p className="text-[10px] font-bold text-white/40">{n.fromAvatar||"❤️"} {n.type.toUpperCase()} • {userData?.avatar||"❤️"} Clean • Private</p><p className="text-[13px] text-white mt-1">{n.text} {n.fromAvatar||"❤️"} Clean</p><p className="text-[10px] text-white/20 mt-2">{n.createdAt?.toDate?.().toLocaleString?.()||'Just now'} {userData?.avatar||"❤️"}</p></div>)}{notifications.length===0 && <div className="py-16 text-center"><p className="font-bold text-white/40">No notifications as {userData?.anonymousName} {userData?.avatar||"❤️"} Yet Clean Only Hearts</p></div>}<button onClick={()=>setShowNotifications(false)} className="w-full mt-6 bg-white text-black h-12 rounded-full font-bold text-xs">Close as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button></div></div></div>}
      ...
      {reportingPost && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[60] flex items-end justify-center p-4">
          <div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 pb-8">
            ...
            <h3 className="font-black text-[16px] text-white">Report as {userData?.anonymousName} {userData?.avatar||"❤️"} - Only Hearts Clean No Vulgar</h3>
            <p className="text-[11px] text-white/40 mt-1">{reportingPost.text.slice(0,60)}... {reportingPost.avatar||"❤️"} Text only Clean</p>
            <p className="text-[10px] font-bold tracking-widest text-white/30 mt-5">SELECT REASON {userData?.avatar||"❤️"} CLEAN ONLY HEARTS</p>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={()=>handleReport(reportingPost)} disabled={!reportReason} className={`flex-1 h-12 rounded-full font-bold text-xs ${!reportReason?'bg-white/5 text-white/20 border-2 border-white/5':'bg-red-500 text-white'}`}>Report as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAdmin && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[70] flex flex-col">
          <div className="max-w-[600px] mx-auto w-full flex-1 flex flex-col bg-[#0a0a0b] p-4 overflow-y-auto">
            <div className="flex justify-between items-center"><h2 className="font-black text-white">ADMIN {userData?.avatar||"❤️"} ONLY HEARTS - {adminReports.length} Pending - {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</h2><button onClick={()=>setShowAdmin(false)} className="w-8 h-8 bg-white/10 rounded-full text-white">X</button></div>
            <div className="mt-6 space-y-4">
              {adminReports.map((rep:any)=><div key={rep.id} className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4">
                ...
                <p className="text-[13px] text-white mt-3">{rep.yakText} {rep.reporterAvatar||"❤️"} Clean Only Hearts</p>
                <div className="flex gap-2 mt-4">
                  ...
                  <button onClick={()=>handleAdminDelete(rep)} className="flex-1 h-10 bg-red-500 text-white rounded-full text-[11px] font-bold">Delete {rep.reporterAvatar||"❤️"} Clean</button>
                </div>
              </div>)}
              {adminReports.length===0 && <p className="text-center text-white/20 py-20">No pending reports {userData?.avatar||"❤️"} All clean Only Hearts Clean</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[80] flex items-center justify-center p-4">
          <div className="bg-[#141416] border-2 border-white/10 w-full max-w-[360px] rounded-[24px] p-6 shadow-2xl">
            ...
            <p className="text-[11px] text-white/40 text-center mt-2 leading-[1.5]">Are you sure you want to logout as {userData?.anonymousName} {userData?.avatar||"❤️"}?<br/>Your anonymous posts as {userData?.avatar||"❤️"} & crushes stay safe<br/>Text Only Clean No Vulgar • Only Hearts • Delete anytime {userData?.avatar||"❤️"} Clean</p>
            <div className="flex gap-3 mt-6">
              <button onClick={()=>setShowLogoutConfirm(false)} className="flex-1 h-12 bg-white text-black rounded-full font-bold text-xs">Cancel {userData?.avatar||"❤️"} Clean</button>
            ...
            <p className="text-[9px] text-white/20 text-center mt-4">Your data as {userData?.anonymousName} {userData?.avatar||"❤️"} stays safe - Only logout - Private chats remain - Only Hearts</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center">
          <div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <h2 className="font-black text-[16px] leading-none text-white">{userData?.anonymousName} {userData?.avatar||"❤️"} SRET ONLY - Only Hearts - Text Only Clean</h2>
                <p className="text-[11px] text-white/40 mt-2 leading-[1.4]">No real name • {userData?.anonymousName} {userData?.avatar||"❤️"} • Text only 300 chars • No pic • No vulgar • Clean • Delete anytime • Only Hearts ❤️🧡💛💚💙💜🤎🖤🤍🩷🩵🩶 • {totalUsers} verified • {dmChats.length} private chats {userData?.avatar||"❤️"} Clean</p>
                <div className="flex gap-2 mt-4 flex-wrap">
                  <span className="px-3 py-1.5 bg-white text-black rounded-full text-[10px] font-bold">{userData?.anonymousName} {userData?.avatar||"❤️"} {userData.yakarma} karma Text Only Clean</span>
                  <span className="px-3 py-1.5 bg-green-500/10 border border-green-500/20 text-green-400 rounded-full text-[9px] font-bold">Private {dmChats.length} {userData?.avatar||"❤️"} Clean</span>
                  <span className="px-3 py-1.5 bg-pink-500 text-white rounded-full text-[9px] font-bold">Matches {crushMatches.filter((c:any)=>c.matched).length} {userData?.avatar||"❤️"} Clean</span>
                  <span className="px-3 py-1.5 bg-white/5 border border-white/10 text-white/40 rounded-full text-[9px] font-bold">{userData?.avatar||"❤️"} ONLY HEARTS CLEAN</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 bg-white/[0.03] border border-white/10 rounded-xl p-3">
              <p className="text-[11px] text-white/60">Selected as {userData?.anonymousName}: <span className="text-white font-bold text-[14px]">{selectedAvatar||userData?.avatar||"❤️"}</span> - Only Hearts Avatar - Text Only Clean No Vulgar Delete Anytime</p>
            </div>
            ...
            <div className="mt-6 bg-white/[0.03] border-2 border-white/10 rounded-[16px] p-4">
              <p className="text-[11px] font-bold text-white/60">WISHES DONE AS {userData?.anonymousName} {userData?.avatar||"❤️"} - ONLY HEARTS:</p>
              <p className="text-[11px] text-white/40 mt-2 leading-[1.6]">
                ✅ Avatar: Only Hearts ❤️🧡💛💚💙💜🤎🖤🤍🩷🩵🩶 - verevi vaddhu<br/>
                ✅ Text Only - No pic tag - Clean<br/>
                ✅ No Vulgar - BAD_WORDS filter - containsVulgar everywhere<br/>
                ✅ Anonymous Name: {userData?.anonymousName} {userData?.avatar||"❤️"} - No real name - 300 chars - No pic<br/>
                ✅ Delete Anytime: DELETE button - Confirm as {userData?.anonymousName} {userData?.avatar||"❤️"} - Clean<br/>
                ✅ Simple and Clarity: Vertical neat - Only Hearts - Clean
              </p>
            </div>

            <button onClick={()=>setShowAdmin(true)} className="w-full mt-4 bg-yellow-500/10 border-2 border-yellow-500/20 h-12 rounded-full text-xs font-bold text-yellow-400">Admin {userData?.avatar||"❤️"} Review Panel - {adminReports.length} Pending {adminReports.length>0?'🔴':''} Clean</button>

            {blockedUsers.length>0 && (
              <div className="mt-6 bg-red-500/5 border-2 border-red-500/10 rounded-[16px] p-4">
                <p className="text-[11px] font-bold text-red-400">BLOCKED USERS {userData?.avatar||"❤️"} - {blockedUsers.length} - Only Hearts Clean</p>
            ...
            <div className="mt-6 bg-red-500/5 border-2 border-red-500/10 rounded-[16px] p-4">
              <p className="text-[11px] font-bold text-red-400">DANGER ZONE {userData?.avatar||"❤️"} - DELETE DATA - {userData?.anonymousName} {userData?.avatar||"❤️"} Text Only Clean - Only Hearts</p>
              <div className="mt-4 space-y-3">
                <div className="bg-white/[0.03] border border-white/10 rounded-xl p-3">
                  <p className="text-[11px] font-bold text-white">Delete Only Posts as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</p>
                  <button onClick={async()=>{ if(!confirm(`Delete posts as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean?`)) return; const uid=user.uid; const yaksSnap = await getDocs(query(collection(db,'yaks'),where('uid','==',uid))); for(const d of yaksSnap.docs){ await deleteDoc(doc(db,'yaks',d.id)); } await updateDoc(doc(db,'users',userData.id),{totalPosts:0, yakarma:100}); showToast(`Posts deleted as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean`); }} className="w-full mt-3 h-10 bg-white/5 border border-white/10 rounded-full text-[11px] font-bold text-white/60">Delete My Posts Only as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean</button>
                </div>
                <div className="bg-red-500/5 border border-red-500/10 rounded-xl p-3">
                  <p className="text-[11px] font-bold text-red-400">Delete Account + All Data as {userData?.anonymousName} {userData?.avatar||"❤️"} Forever Clean</p>
                  <button onClick={async()=>{
                    const confirm1 = confirm(`DELETE ACCOUNT as ${userData?.anonymousName} ${userData?.avatar||"❤️"}? Only Hearts Clean\n\nDeletes Profile and All posts\nCannot be undone - ${userData?.avatar||"❤️"} Clean`);
                    if(!confirm1) return;
                    const confirm2 = prompt(`Type "DELETE" as ${userData?.anonymousName} ${userData?.avatar||"❤️"} to confirm Clean`);
                    if(confirm2!=='DELETE'){ showToast(`Cancelled as ${userData?.anonymousName} ${userData?.avatar||"❤️"} Clean`); return; }
                    try{
                  ...
                  }} className="w-full mt-3 h-10 bg-red-500 text-white rounded-full text-[11px] font-bold">Delete Account {userData?.anonymousName} {userData?.avatar||"❤️"} + All Data Forever Clean</button>
                </div>
              </div>
            </div>

            <button onClick={()=>setShowLogoutConfirm(true)} className="w-full mt-6 bg-white/5 border-2 border-white/10 h-12 rounded-full text-xs font-bold text-white/60">Logout as {userData?.anonymousName} {userData?.avatar||"❤️"} - SRET ONLY - Only Hearts Clean</button>
            <button onClick={()=>setShowProfile(false)} className="w-full mt-3 bg-white text-black h-12 rounded-full font-bold text-xs">Close as {userData?.anonymousName} {userData?.avatar||"❤️"} Clean Only Hearts</button>
