...
import { Home, Timer, Target, BarChart3, User, Flame, Clock, Trophy, Play, Check } from 'lucide-react';
        ...
        if(isFocusing){
          ...
          return {
           ...prev,
            ...
            lifeScore: Math.max(0, 100 - Math.floor(prev.screenTime/600))
          }
        }
      });
    }, 1000);
      ...
      <div className="p-4 max-w-2xl mx-auto">
        ...
        {tab==='focus' && <div className="space-y-6 text-center">
          ...
          {isFocusing? (
            ...
            <div className="space-y-4">
              ...
              {[25, 50, 90].map(mins => (
                <button key={mins} onClick={() => startFocus(mins)} className="w-full bg-gray-900 border border-gray-800 py-4 rounded-xl text-xl font-bold hover:bg-gray-800">
                  {mins} Minutes
                </button>
              ))}
            </div>
        ...
        {tab==='profile' && <div className="space-y-4">
          ...
          <div className="bg-gray-900/50 p-5 rounded-2xl">
            ...
            {data.badges.map(b => <p key={b} className="bg-yellow-600/20 text-yellow-400 px-3 py-1 rounded-full inline-block mr-2">{b}</p>)}
          </div>

          <div className="bg-gray-900/50 p-5 rounded-2xl">
            <h3 className="font-bold mb-2">Goals</h3>
            <p>Screen Time < 2h Daily</p>
          </div>
