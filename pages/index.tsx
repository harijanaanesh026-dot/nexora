...
const BRANCHES = ["CSE", "ECE", "EEE", "MECH", "CIVIL", "AIML"];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      data = data.filter(y =>!blocked.includes(y.uid));
    ...
    return () => unsub();
  }, [userData, feedType, selectedTopic, search, blocked]);
  ...
  const handleImages = (e: any) => {
    ...
    files.forEach(f => {
      ...
      reader.onloadend = () => setImages(p => [...p, reader.result as string].slice(0, 4));
  ...
  const handlePost = async () => {
    if (!newYak.trim() && images.length === 0) {
      alert('Emanna rayi bro!');
      return;
    }
    if (!user ||!userData) {
      alert('User data ledu, refresh chey');
      return;
    }
    try {
  ...
  const th = darkMode
   ? { bg: 'bg-black', card: 'bg-zinc-900', input: 'bg-zinc-800', text: 'text-white', sub: 'text-zinc-400', border: 'border-zinc-800' }
  ...
  if (screen === 'login') {
    return (
      <div className={`min-h-screen ${th.bg} ${th.text} flex flex-col items-center justify-center p-6`}>
        ...
        <button onClick={() => signInWithPopup(auth, provider)} className="mt-8 bg-yellow-400 text-black px-8 py-3 rounded-full font-bold">Continue with Google</button>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen ${th.bg} ${th.text} pb-20`}>
      <div className={`sticky top-0 ${darkMode? 'bg-black' : 'bg-white'} border-b ${th.border} p-3 z-10`}>
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <div className={`${th.input} px-3 py-1 rounded-full text-xs`}>👤 {userData?.username}</div>
          </div>
        </div>
      ...
      <div className="p-3 space-y-3">
        {yaks.length === 0 && <p className={`text-center mt-10 ${th.sub}`}>No posts yet. Be first! 👻</p>}
        {yaks.map(y => {
            ...
            <div key={y.id} className={`${th.card} rounded-2xl p-4 border ${th.border}`}>
              <div className="flex justify-between text-[11px] mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  {isOwner && <button onClick={() => handleDeletePost(y)} className="text-red-400 bg-red-500/10 px-2 rounded-full">🗑️ Delete</button>}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <div className={`flex ${th.input} rounded-full`}>
              ...
              {activeCommentPost === y.id && (
                <div className="mt-4 pt-3 border-t border-zinc-800">
                  ...
                  {comments.filter(c =>!c.parentId).map(c => (
                    <div key={c.id} className="mb-3">
                      <div className="flex justify-between">
                        <p className="text-[13px]"><b className={th.sub}>{c.username}</b>: {c.text}</p>
                        {user.uid === c.uid && <button onClick={() => handleDeleteComment(y.id, c.id)} className="text-red-400 text-xs">🗑️</button>}
                      </div>
                      <div className="flex gap-3 mt-1 text-[11px]">
                        <button onClick={() => handleVoteComment(y.id, c, 'likes')}>⬆️ {c.likes || 0}</button>
                        <button onClick={() => handleVoteComment(y.id, c, 'dislikes')}>⬇️ {c.dislikes || 0}</button>
                        <button onClick={() => setReplyTo(c)} className="text-yellow-400 font-bold">Reply</button>
                      </div>
                      <div className="ml-4 mt-2 border-l border-zinc-700 pl-3 space-y-2">
                        {comments.filter(r => r.parentId === c.id).map(r => (
                          <div key={r.id} className="flex justify-between">
                            <p className="text-[12px]"><span className="text-yellow-400">↳ @{r.replyToUsername}</span> <b>{r.username}</b>: {r.text}</p>
                            <div className="flex gap-2">
                              <button onClick={() => handleVoteComment(y.id, r, 'likes')} className="text-[11px]">⬆️ {r.likes || 0}</button>
                              {user.uid === r.uid && <button onClick={() => handleDeleteComment(y.id, r.id)} className="text-red-400 text-xs">🗑️</button>}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex gap-2 mt-3">
                    <input value={commentText} onChange={e => setCommentText(e.target.value)} placeholder={replyTo? `Reply to ${replyTo.username}...` : 'Add comment...'} className={`flex-1 ${th.input} rounded-full px-4 py-2 text-sm outline-none`} />
                    <button onClick={() => handleAddComment(y)} className="bg-yellow-400 text-black px-5 rounded-full font-bold text-sm">Post</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {screen === 'create' && (
        <div className={`fixed inset-0 ${th.bg} z-20 p-4`}>
          ...
          <textarea value={newYak} onChange={e => setNewYak(e.target.value)} placeholder="What's happening in campus?" className={`w-full h-32 mt-3 p-3 ${th.input} rounded-xl`} />
          <label className={`w-full mt-3 border border-dashed border-yellow-400 p-3 rounded-xl flex justify-center text-sm ${th.input}`}>🖼️ Add Images<input type="file" multiple hidden accept="image/*" onChange={handleImages} /></label>
