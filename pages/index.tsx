  ...
  useEffect(() => {
    ...
    agoraClient.on("user-published", async (user: any, mediaType: string) => {
      ...
      if (mediaType === "video") {
        ...
        setTimeout(() => { const player = document.getElementById(`player-${user.uid}`); if(player) user.videoTrack?.play(player); }, 100);
      }
      if (mediaType === "audio") user.audioTrack?.play();
    });
    agoraClient.on("user-unpublished", (user: any) => { setRemoteUsers((prev) => prev.filter((u) => u.uid!== user.uid)); });
    init();
    return () => { localTracks.audio?.close(); localTracks.video?.close(); agoraClient.leave(); };
...
export default function Nexora() {
  ...
  useEffect(() => {
    // FIX: 1 SEC DELAY PETTI REDIRECT RESULT CHECK CHESTHAM
    setTimeout(() => {
      getRedirectResult(auth).then((result) => {
        if(result) toast.success("Welcome to Nexora!");
      }).catch((error) => {
        console.log(error);
      });
    }, 1000);
  ...
  const handleGoogleLogin = async () => { await signInWithRedirect(auth, googleProvider); };
    ...
    <div className="h-screen flex items-center justify-center bg-black text-white p-4">
      <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="w-full max-w-sm p-6 bg-gray-900 rounded-xl text-center">
        ...
        <button onClick={handleGoogleLogin} className="w-full bg-white text-black p-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-gray-200">
          <svg className="w-5 h-5" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 2.053 29.268 0 24 0 10.745 0 0 5.373 0 12s10.745 24 24 24 24-10.745 24-24c0-1.658-.188-3.27-.533-4.917z"/><path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 2.053 29.268 0 24 0 16.318 0 9.656 4.245 6.306 10.691z"/><path fill="#4CAF50" d="M24 48c6.48 0 12.265-2.36 16.808-6.295l-6.522-5.025C32.639 39.017 28.515 40 24 40c-5.202 0-9.929-2.002-13.478-5.273l-6.52 5.027C7.689 43.911 15.469 48 24 48z"/><path fill="#1976D2" d="M43.611 20.083H24v8h11.303c-.792 2.237-2.231 4.166-4.087 5.571l6.522 5.025C42.9 35.479 44 28.985 44 24c0-1.658-.188-3.27-.533-4.917z"/></svg>
          Sign in with Google
        </button>
      </motion.div>
    </div>
