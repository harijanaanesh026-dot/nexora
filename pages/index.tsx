...
const Footer = () => (<div className="w-full py-8 flex flex-col items-center gap-1 border-t border-white/[0.06] mt-8"><p className="text-[10px] tracking-[0.3em] font-bold text-white/40">SRET ONLY • A PRODUCTION BY ANESH</p><p className="text-[9px] text-white/20">Premium Campus • Crush + Market + PYQ</p></div>);
...
export default function YakFixed(){
  ...
  const handleCrushSubmit=async()=>{
    ...
    try{
      // check if other user exists with that roll
      const snap=await getDocs(query(collection(db,'users'),where('rollNumber','==',roll)));
      if(snap.empty){ await addDoc(collection(db,'crushes'),{fromUid:user.uid, fromRoll:userData.rollNumber||'ANON', toRoll:roll, matched:false, createdAt:serverTimestamp()}); showToast("Crush saved secretly 💜 - If they crush you back, you'll match!"); setCrushRoll(''); return; }
      ...
      if(!mutualSnap.empty){
        // MATCH!
        await addDoc(collection(db,'crushes'),{fromUid:user.uid, fromRoll:userData.rollNumber, toRoll:roll, toUid:other.uid, matched:true, createdAt:serverTimestamp()});
        await updateDoc(doc(db,'crushes',mutualSnap.docs[0].id),{matched:true});
        await createNotification(other.uid, 'crush', `💘 You have a SECRET MATCH! Someone crushed you back - Check Crush tab!`);
        showToast("💘 IT'S A MATCH! You both crushed each other!");
      }else{
        await addDoc(collection(db,'crushes'),{fromUid:user.uid, fromRoll:userData.rollNumber, toRoll:roll, toUid:other.uid, matched:false, createdAt:serverTimestamp()});
        showToast("Crush saved secretly 💜 - If they crush you back, you'll match!");
      }
      setCrushRoll('');
    }catch(e:any){ showToast(e.message); }
  };
    ...
    <div className="min-h-screen bg-[#0a0a0b] text-white flex flex-col">
      ...
      <div className="sticky top-0 z-20 bg-[#0a0a0b]/80 backdrop-blur-2xl border-b border-white/10">
        ...
        <div className="max-w-[600px] mx-auto px-3 pb-3 flex gap-2 overflow-x-auto">
          ...
          <button onClick={()=>setFeedTab('crush')} className={`h-9 px-4 rounded-full text-xs font-bold border-2 whitespace-nowrap ${feedTab==='crush'?'bg-pink-500 text-white border-pink-500':'bg-white/5 border-white/10 text-white/40'}`}>💘 CRUSH {crushMatches.filter((c:any)=>c.matched).length}</button>
          <button onClick={()=>setFeedTab('market')} className={`h-9 px-4 rounded-full text-xs font-bold border-2 whitespace-nowrap ${feedTab==='market'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>🛒 MARKET {marketYaks.length}</button>
          <button onClick={()=>setFeedTab('pyq')} className={`h-9 px-4 rounded-full text-xs font-bold border-2 whitespace-nowrap ${feedTab==='pyq'?'bg-white text-black border-white':'bg-white/5 border-white/10 text-white/40'}`}>📚 PYQ {pyqYaks.length}</button>
      ...
      <div className="max-w-[600px] mx-auto w-full flex-1 p-3 pb-[84px] space-y-3">
        ...
        {feedTab==='crush' && (
          <div className="space-y-3">
            <div className="bg-gradient-to-br from-pink-500/10 to-purple-500/10 border-2 border-pink-500/20 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-pink-400">PREMIUM - CRUSH MATCHER 💘</p><h3 className="font-black mt-2 text-white">Secret Crush - SRET Only</h3><p className="text-[11px] text-white/40 mt-1">Enter roll number secretly. If both crush each other - MATCH!</p><div className="flex gap-2 mt-4"><input value={crushRoll} onChange={e=>setCrushRoll(e.target.value.toUpperCase())} placeholder="Crush Roll e.g. 21CS105" className="flex-1 bg-black/30 border-2 border-pink-500/20 rounded-full px-5 h-11 text-[13px] outline-none text-white placeholder:text-white/30 focus:border-pink-500"/><button onClick={handleCrushSubmit} className="px-6 h-11 bg-pink-500 text-white rounded-full font-bold text-xs">Crush 💜</button></div></div>
            {crushMatches.filter((c:any)=>c.matched).length>0 && <div className="bg-pink-500/10 border-2 border-pink-500/30 rounded-[18px] p-4"><p className="text-[10px] font-bold text-pink-400">💘 YOUR MATCHES - {crushMatches.filter((c:any)=>c.matched).length}</p>{crushMatches.filter((c:any)=>c.matched).map((m:any)=><div key={m.id} className="mt-3 bg-white/5 p-3 rounded-xl flex justify-between items-center"><div><p className="font-bold text-[13px] text-white">MATCH! 🎉 {m.toRoll}</p><p className="text-[10px] text-white/40">You both like each other - DM now?</p></div><button onClick={()=>handleStartDm(m.toUid)} className="px-4 py-2 bg-white text-black rounded-full text-[11px] font-bold">DM 💬</button></div>)}</div>}
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4"><p className="text-[10px] font-bold text-white/30">YOUR SECRET CRUSHES - {crushMatches.length}</p>{crushMatches.map((m:any)=><div key={m.id} className="mt-2 flex justify-between items-center bg-white/[0.02] p-3 rounded-xl"><p className="text-[13px] text-white/60">{m.toRoll} {m.matched?'💘 MATCHED':'🤫 Secret'}</p><span className={`text-[10px] px-2 py-1 rounded-full ${m.matched?'bg-pink-500 text-white':'bg-white/10 text-white/40'}`}>{m.matched?'MATCH':'PENDING'}</span></div>)}{crushMatches.length===0 && <p className="text-[11px] text-white/20 mt-3 text-center py-6">No crushes yet - Add your SRET crush roll number secretly 👻</p>}</div><Footer/>
          </div>
        )}
        {feedTab==='market' && (
          <div className="space-y-3">
            ...
            {marketYaks.map((y:any)=><div key={y.id} className="bg-white/[0.04] border-2 border-white/10 rounded-[20px] p-5"><div className="flex justify-between"><p className="font-bold text-[13px] text-white">Anonymous - SRET - 🛒 MARKET</p><span className="px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-full text-[11px] font-bold text-green-400">₹{y.price}</span></div><p className="text-[14px] text-white mt-3">{y.text}</p>{y.image && <img src={y.image} className="mt-3 rounded-[16px] border-2 border-white/10 w-full max-h-[300px] object-cover"/>}<button onClick={()=>handleStartDm(y.uid, y.id)} className="mt-4 w-full h-10 bg-white text-black rounded-full font-bold text-xs">DM to Buy 💬</button></div>)}
            {marketYaks.length===0 && <div className="py-16 text-center bg-white/[0.03] border-2 border-white/10 rounded-[24px]"><p className="text-3xl">🛒</p><p className="font-black mt-3 text-white">No items yet</p><p className="text-[11px] text-white/30">Sell your books anonymously</p></div>}<Footer/>
          </div>
        )}
        {feedTab==='pyq' && (
          <div className="space-y-3">
            <div className="bg-white/[0.05] border-2 border-white/10 rounded-[20px] p-5"><p className="text-[10px] font-bold tracking-[0.2em] text-white/30">SRET PYQ VAULT - PREMIUM 📚</p><p className="font-black mt-1 text-white">Previous Year Questions - Anonymous Share</p><p className="text-[11px] text-white/40 mt-1">Upload PYQs - Help juniors - Earn karma</p><button onClick={()=>{setYakType('pyq'); setScreen('create');}} className="mt-3 bg-white text-black px-5 h-9 rounded-full text-xs font-bold">Upload PYQ +</button></div>
            {pyqYaks.map((y:any)=><div key={y.id} className="bg-white/[0.04] border-2 border-white/10 rounded-[20px] p-5"><div className="flex gap-2 items-center"><span className="px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full text-[10px] font-bold text-blue-400">{y.subject||'PYQ'}</span><span className="text-[10px] text-white/30">{y.hashtags?.join(' ')}</span></div><p className="text-[14px] text-white mt-3">{y.text}</p>{y.image && <img src={y.image} className="mt-3 rounded-[16px] border-2 border-white/10 w-full max-h-[400px] object-contain"/>}<div className="flex gap-2 mt-4"><button onClick={()=>handleVote(y,'up')} className="px-4 h-8 bg-white/5 border border-white/10 rounded-full text-xs text-white/40">Up {y.likes||0}</button><button onClick={()=>handleStartDm(y.uid)} className="px-4 h-8 bg-white text-black rounded-full text-xs font-bold">Get PDF DM 💬</button></div></div>)}
            {pyqYaks.length===0 && <div className="py-16 text-center bg-white/[0.03] border-2 border-white/10 rounded-[24px]"><p className="text-3xl">📚</p><p className="font-black mt-3 text-white">No PYQs yet</p><p className="text-[11px] text-white/30">Upload first PYQ and earn 20 karma</p></div>}<Footer/>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-[#0a0a0b] z-40 flex flex-col overflow-hidden">
          ...
          <div className="max-w-[600px] mx-auto w-full flex flex-col h-full bg-[#0a0a0b]">
            ...
            <div className="p-6 flex-1 overflow-y-auto bg-[#0a0a0b]">
              ...
              {yakType==='market' && <div className="flex gap-2 mb-4"><input value={marketPrice} onChange={e=>setMarketPrice(e.target.value)} placeholder="Price e.g. 250" className="w-[120px] p-4 bg-white/[0.03] border-2 border-green-500/20 rounded-xl text-sm outline-none text-white placeholder:text-white/30"/><input value={pyqSubject} onChange={e=>setPyqSubject(e.target.value)} placeholder="Item: e.g. Drafter, Books" className="flex-1 p-4 bg-white/[0.03] border-2 border-white/10 rounded-xl text-sm outline-none text-white placeholder:text-white/30 hidden"/><p className="text-[10px] text-green-400 flex items-center">₹ Price</p></div>}
      ...
      {showProfile && (<div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end justify-center"><div className="bg-[#141416] border-2 border-white/10 w-full max-w-[600px] rounded-t-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl"><div className="w-10 h-1 bg-white/10 rounded-full mx-auto mb-6"></div><div className="flex gap-4"><div className="w-[72px] h-[72px] bg-white/5 border-2 border-white/10 rounded-[20px] flex items-center justify-center text-3xl text-white">S</div><div className="flex-1"><h2 className="font-black text-[16px] leading-none text-white">SRET ONLY - PREMIUM CAMPUS</h2><p className="text-[11px] text-white/40 mt-2 leading-[1.4]">Crush + Market + PYQ + DM + Awards - SRET ONLY - {totalUsers} verified - {dmChats.length} chats - {crushMatches.filter((c:any)=>c.matched).length} matches</p><div className="flex gap-2 mt-4 flex-wrap"><span className="px-3 py-1.5 bg-white text-black rounded-full text-[10px] font-bold">{userData.yakarma} karma</span><span className="px-3 py-1.5 bg-pink-500 text-white rounded-full text-[9px] font-bold">💘 {crushMatches.filter((c:any)=>c.matched).length} MATCHES</span><span className="px-3 py-1.5 bg-white/10 border border-white/10 text-white rounded-full text-[9px] font-bold">DM {dmChats.length} • NOTIF {notifications.length}</span></div></div></div>
      <div className="grid grid-cols-4 gap-3 mt-6"><div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 text-center"><p className="font-black text-xl text-white">{userData.totalPosts||0}</p><p className="text-[9px] font-bold tracking-widest text-white/30 mt-1">POSTS</p></div><div className="bg-white/[0.05] border-2 border-white/10 rounded-[18px] p-4 text-center"><p className="font-black text-xl text-white">{totalUsers}</p><p className="text-[9px] font-bold tracking-widest text-white/30 mt-1">VERIFIED</p></div><div className="bg-pink-500/10 border-2 border-pink-500/20 rounded-[18px] p-4 text-center"><p className="font-black text-xl text-pink-400">{crushMatches.filter((c:any)=>c.matched).length}</p><p className="text-[9px] font-bold tracking-widest text-pink-400/60 mt-1">MATCHES</p></div><div className="bg-white text-black rounded-[18px] p-4 text-center"><p className="font-black text-xl">{userData.yakarma}</p><p className="text-[9px] font-bold tracking-widest mt-1">KARMA</p></div></div>
      ...
      <div className="mt-6 bg-white/[0.03] border border-white/10 rounded-[16px] p-4"><p className="text-[11px] font-bold text-white">PREMIUM FEATURES - SRET ONLY:</p><p className="text-[11px] text-white/40 mt-2 leading-[1.6]">1. 💘 Crush Matcher - Secret roll enter chey - Mutual aithe MATCH - Both anonymous!<br/>2. 🛒 Marketplace - Books, drafter, lab coat sell - Anonymous DM to buy<br/>3. 📚 PYQ Vault - Previous papers share - Juniors ki help - Karma +20<br/>4. DM + Hashtag + Notifications + Weekly Awards<br/>5. Search + Trending + Leaderboard</p></div>
